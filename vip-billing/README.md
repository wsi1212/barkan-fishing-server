...
2. 이용 기간은 1개월·1년(각 30·365일) 중 선택한다. 화면의 기간·금액과 주문의 `period_days`는 `membership-periods.mjs`의 동일한 기준표에서 만든다.
...
1. 결제 화면에서 30일·365일만 표시되고 위 요금표와 일치하는지 확인한다.
2. 계좌이체 주문을 두 기간으로 한 번씩 만든 뒤 운영자 도구의 대기 주문에서 `period_days`가 선택값과 같은지 확인하고, 입금 확인 후 응답 메시지와 구독 만료일을 확인한다.
