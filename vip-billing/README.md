...
2. 이용 기간은 1·3·5개월과 1년(각 30·90·150·365일) 중 선택한다. 1·3·5개월은 월간 요금에 개월 수를 곱하고, 1년만 별도 할인한다. 화면의 기간·금액과 주문의 `period_days`는 `membership-periods.mjs`의 동일한 기준표에서 만든다.
...
5. 입금 확인 대상의 마크 계정이 Discord와 연결돼 있으면 같은 DB 트랜잭션에서 멤버십 역할 작업을 큐에 넣는다. Discord 봇이 5초마다 처리하며 `VIP`/`MVP`/`MVP+` 중 활성 등급 하나만 유지한다.
...
1. 결제 화면에서 1·3·5개월·1년만 표시되고, 1·3·5개월은 월간 요금의 배수인지 위 요금표와 일치하는지 확인한다.
2. 계좌이체 주문을 네 기간으로 한 번씩 만든 뒤 운영자 도구의 대기 주문에서 `period_days`가 선택값과 같은지 확인하고, 입금 확인 후 응답 메시지와 구독 만료일을 확인한다.
