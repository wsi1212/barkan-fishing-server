...
| **배** | `ship/` (ShipManager·ShipFactory·ShipMover) + `model/` + `command/ShipCommandManager` + `editor/ShipEditor` | 프리셋 **1종 `돛단배`**(2026-09-05 `범선` 폐지·교체, 소유는 PlayerShipData 가 읽을 때 자동 이관). 선체=구운 ItemDisplay 1개(`ship-models.json`)+돛만 BlockDisplay, 충돌=발밑 카펫. ★**멈추면 실제 블록이 된다** — 도킹이 끝나(정수격자+90°) 2초 지나면 `ship/ShipDockBlocks` 가 청사진을 월드에 놓고 선체 디스플레이를 숨긴다(전/후진 입력에 그 틱 되돌림). 덮는 칸은 공기·물·해초뿐이고, 놓기 전에 `ship-docked.json` 저널을 남겨 크래시로 남은 선체를 다음 기동 첫 틱에 되돌린다. 정박 중 파괴·설치·보관함은 차단(부수면 무한 복제). 끄기=`config.yml` `blockship.dock.real-blocks: false`. ★블루프린트는 손편집 금지 — `imugi-boss/scan_ship_world.py <프리셋>`(월드에서 재추출) → `bake_ship.py <프리셋> <영문명>` |
...
- **「가이드」 칭호 보유자는 OP 없이 BlockShip 관리 명령을 쓴다.** 판정은 **보유**다(장착 아님 — `moderation/ChatMuteManager` 가 세운 규칙). 진짜 OP 로 만들지 않고 `guide/GuideAccess` 가 `PermissionAttachment` 로 관리 명령 노드와 안내용 바닐라 TP 노드(`minecraft.command.teleport`)를 붙인다. 가이드는 `/tp`·`/teleport`만 사용할 수 있고, 바닐라 `/op`·`/gamemode`·`/give`·`/execute`와 네임스페이스 우회는 계속 막힌다.
...
- 조회용 하위 명령은 기록되지 않는다(`/콤보` 는 조용, `/콤보 7` 은 기록). 도감 열기·페이지 조회도 조용히 하고, **도감에서 실제 테스트 아이템을 꺼내 지급받는 클릭만 기록**한다. 귓말·`/채팅`·`/길드`·`/섬 방문`처럼 OP 없이 가능한 일반 기능도 기록하지 않는다. 탭 자동완성도 기록 안 함. **차단된 시도(🚫)도 같은 채널로 간다** — 권한 상승 시도는 성공한 명령보다 먼저 봐야 한다.
- **치명적인 명령은 코드에 하드코딩해 막았다** (`GuideAccess.HARD_DENY`, 60규칙). config 가 아닌 이유는 지우는 실수로 뚫리면 안 되는 경계라서다. 막은 갈래: ①권한 증식(`칭호 부여/회수/생성/전체지급`·`구독`) ②재화·아이템 생성(`돈 설정`·`캐시지급`·`*지급` 전부·`보물상자`·`플라이 추가/발권`) ③**`낚시테스트`·`낚시발동`** — 이름은 테스트지만 다음 어획 «등급·발동을 강제»해 최고등급을 무한 생산한다 ④진행도 조작·초기화(`레벨설정/초기화`·`퀘스트강제완료/클리어/초기화`·`스탯관리`·`도감관리`·`섬 초기화`·`콤보 *`) ⑤되돌릴 수 없는 삭제·서버 전체(`아이템청소`·`npc해제`·`페리삭제`·`지역 생성/삭제/설정`·`잠긴문`(마스터키)·`이무기`·`랜덤블럭10`·`name`(희귀템 사칭)). 남의 소지품·보관함 편집은 가이드에게도 허용하되 명령 실행이 감사 로그에 남는다.
...
- **바닐라는 TP만 가이드에게 열린다** — 가이드의 `/tp`·`/teleport`는 허용하되 `/op`·`/stop`·`/deop`·`/ban`·`/kick`·`/whitelist`·`/gamemode`·`/give`·`/execute`와 네임스페이스 우회(`/minecraft:tp` 포함)는 계속 차단.
...
- **`send_command_feedback=false` 는 의도된 설정이다 (2026-09-06, prod·dev 양쪽).** 우리 워프(페리·포탈·섬·여관·잠수·지역이동)가 전부 콘솔 `execute in <dim> run tp` 라 바닐라가 «…을(를) …(으)로 순간이동시켰습니다» 를 접속 중인 **모든 OP** 에게 뿌렸다(`CommandSourceStack#broadcastToAdmins` — 유일한 게이트가 이 게임룰). 부작용으로 OP 가 직접 친 바닐라 명령의 **성공** 메시지도 안 뜬다(실패 메시지는 그대로) — 유저가 그래도 좋다고 결정했다. 디스패치 순간에만 껐다 되돌리는 코드(util/Tp)는 «부하만 는다» 는 이유로 폐기됐으니 되살리지 말 것. `true` 로 되돌리면 도배가 그대로 재발한다.
  - ★**이 게임룰은 «월드별»이다 — 메인월드만 끄면 절반만 꺼진다.** `execute in <dim>` 이 소스의 월드를 목적지로 바꾸므로 판정은 «도착 월드»의 값으로 난다. 그래서 09-06 오전까지도 크로스월드 워프(페리·포탈·섬·여관·잠수·시승)는 그대로 OP 전원에게 떴다. 전 월드에 `execute in minecraft:<월드> run gamerule send_command_feedback false` 를 돌려 껐고, 재발 방지는 `BlockShipPlugin.applyGlobalWorldRules()`(부팅 + 이후 월드 로드) 가 맡는다 — 나중에 처음 만들어지는 월드(`ship_preview` 등)까지 여기서 걸린다. 확인은 반드시 «도착 월드»로: `execute in minecraft:island_world run gamerule send_command_feedback`.
...
- **게이트 10종을 순서대로 돈다** — 인스턴스데이터 제외목록 · 목표id대조 · **사본드리프트(`audit-copies.py`)** · 퀘스트감사(ERROR 0 기준) · 굵은포맷 · 타임존 · NPC대사 · **빌드출처(`guard-build-source.sh`)** · staged JSON 검증 · **배포후 커밋 대조**. 통과 못 하면 배포가 멈춘다. `SKIP_QUEST_AUDIT` 우회구는 폐지됐다.
  ...
  - ★**스테이징 전에는 미커밋 변경을 반드시 사용자에게 함께 보고한다.** `git status --short` 전체 결과에서 다른 세션이 명시적으로 제외하라고 한 파일만 빼고, 나머지는 관련 없다고 추정해 빼지 말고 스테이징 포함 여부를 확인한다.
