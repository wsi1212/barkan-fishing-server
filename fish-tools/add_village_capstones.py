...
■ 램프
  마을 순서대로 어보가 굵어진다 — 물이 넓어지니까 자연히 그렇게 된다.
      ...
      스폰마을  강·강_상류            → 14칸   (노인 — 평생 민물만 판 사람)
      사막마을  오아시스·붉은사막·붉은호수 → 15칸   (사피르 — 이미 사막 도감 퀘 담당)
      상단마을  상단마을·대양·항구       → 16칸   (알도 — 자칭 낚시광, 도감 퀘 담당)
      왕도     위 전부                → 19칸   (견습 사서 니나 — 왕립 어보 편찬)
...
def catchable(regs):
    ...
    for r in regs:
        ...
        for bucket in REGIONS[r].values():
            sp.update(bucket)
    return len(sp - {s for s in sp if FISH.get(s, {}).get("quest")})
...
CAPSTONES = [
    ...
    dict(id="사사이드_사피르05", giver="사피르", after="사사이드_사피르04", lv=45,
         ...
         설명=lambda n: [
             ...
             f"&7&f오아시스·붉은사막·붉은호수의 물고기 {n}종&7을 전부 등록하세요.",
             "&8붉은 호수 쪽은 아직 아무도 제대로 세어 본 적이 없습니다."],
         대화=(["부탁이 하나 있어요. 제 평생 연구이기도 하고요.",
               "물이 적은 땅일수록 목록이 중요해요. 하나 사라지면 바로 티가 나니까요.",
               "오아시스와 붉은사막, 그리고 붉은 호수까지 — 전부요."],
...
print(f"\n{'(드라이런 — 저장 안 함)' if DRY else '✓ 완료.'} "
      "★다음: build_ch7_quests.py → add_quest_difficulty.py")
