...
for qid, e in QUESTS.items():
    # 주간 풀은 이 규칙 밖이다 (2026-08-17, 유저 결정).
    # `주간_전설낚시`(Lv15)·`주간_전설쌍`(Lv20)은 이 스크립트보다 먼저 있던 반복 퀘스트고
    # 「전설」이라는 이름 자체가 S를 전제한다. 등급을 내리거나 레벨을 올리는 대신 검증에서 뺀다.
    # ★남는 위험 — 주간은 필요레벨로만 게이팅되므로 Lv15 유저에게 배정될 수 있다.
    #   S를 못 잡으면 그 주 목록에 못 깨는 칸이 하나 남는다(길드필요 게이트가 푸는 문제와 같은 계열).
    if e.get("카테고리") == "주간":
        continue
    cap = cap_for(e["필요레벨"])
    if cap is None:
