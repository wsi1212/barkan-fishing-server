...
────────────────────────────────────────────────────────────────────────────
  ...
  §8.1 낚싯대 주스탯 = 난이도(하한 E0 D1 C1 B3 A5 S7 / 상한=숙련형 D2 C3 B5 A7 S9) — 낚싯대 전용
       ★2026-09-09 사다리 재배정 — zone = 8 + floor(net/2) 라 «2점 = 존폭 1칸»이다. 하한을
         등급마다 2씩 올려 등급당 정확히 한 칸을 주고, 상한을 숙련형 값(=하한+2)에 맞췄다.
         구값: 하한 B2 A3 S5 / 상한 D1 C2 B3 (A7 S9 는 09-07 에 이미 비준).
         계기: ecb0509(S 승격 11종)에서 히든 A 6종을 S로 올릴 때 FLOOR 재적용을 빼먹어
         S 8종 전부가 A 하한(3~4)에 앉아 있었다.
...
SUB_BAND = {
    ...
    # ★2026-08-31 재조정(유저 지시, gen_part_builds.py와 동일 사유) — 퀘스트 레벨대(18/19)에 맞춤.
    ("사막마을", "B"): ((20, 26), (140000, 210000)),
...
FLOOR = {
    "난이도": {"E": 0, "D": 1, "C": 1, "B": 3, "A": 5, "S": 7},   # ★2026-09-09 하한 B2 A3 S5 → B3 A5 S7
...
CAP = {
    # ★2026-09-07 유저 지시 — 숙련형 사다리가 B 5 에서 평평했다(A5/S5). B 는 그대로 두고
    #   A 7 · S 9 로 올린다. S 8 은 S급 숙련형 낚싯대가 카탈로그에 없어 도달 불가 상한이었다.
    "난이도": {"E": 0, "D": 2, "C": 3, "B": 5, "A": 7, "S": 9},   # ★2026-09-09 D1 C2 B3 → D2 C3 B5 (상한=하한+2)
...
PRIMARY = {
    "난이도":     {"D": 2,  "C": 3,  "B": 5,  "A": 7,  "S": 9},   # ★2026-09-07 A5→7 · S8→9 / 09-09 D1 C2 B3 → D2 C3 B5
...
# ★「방벽 낚싯대」는 격자 밖 잔존 1종 — 2026-08-28 실측으로 발견(생성기가 멈춰 있었다).
#   격자에 넣으면 스탯이 재계산돼 라이브 값이 바뀌므로 보존하고, 레시피 조성만
#   patch_line_signature.py 가 따로 맞춘다.
# ★2026-08-31 추가 4종 — 「방벽 낚싯대」와 같은 사정(왕도 B, Lv35~38이 LADDER에 없었다).
# ★2026-09-01 추가 6종 — 저티어 히든(스폰 C · 사막 B). 진짜 권위는 patch_hidden_lowtier.py.
#   기본 공식(SUB_BAND·PRIMARY)으로 재계산하면 라이브 사다리보다 약해져 완전열등이 된다
#   — 라이브 C/B 는 patch 레이어가 이미 끌어올려 놨기 때문(잉어꾼 행운 14 ↔ 공식 6).
KEEP_AS_IS = ["잠수부의 낚싯대", "심해 잠수부의 낚싯대", "초보자 낚싯대", "방벽 낚싯대",
              "사자 낚싯대", "왕관 낚싯대", "세관 낚싯대", "문서고 낚싯대",
              "밀물의 낚싯대", "갯바위의 낚싯대", "뱃사공의 낚싯대",
              "사막여우의 낚싯대", "모래시계의 낚싯대", "카라반의 낚싯대",
              # ★2026-09-06 S 승격 6종 — 진짜 권위는 patch_grade_promote_s.py.
              #   부품(릴·줄·바늘·미끼·찌)은 이 시리즈가 전부 S 인데 낚싯대만 A 로 남아
              #   «낚싯대 A + 부품 S» 세트가 되어 있었다. 이 사다리로 재계산하면 A 로
              #   되돌아가므로 값 그대로 보존한다.
              "여명의 낚싯대", "등대지기의 낚싯대", "전갈왕의 낚싯대",
...
PRESERVE_RODS = {
    ...
    # ★2026-08-31 격자 밖 잔존 5종 — 2026-08-27 patch_wangdo_b.py(commit 2dcdf51b)가 왕도
    #   B(Lv35~38)를 신설하며 만든 것들인데, 이 LADDER 는 그 신설을 따라가지 못했다
    #   (gen_part_builds.py 의 왕도 B 「방벽·사자·왕관」 계열과 같은 사정 — 진짜 권위는
    #   patch_wangdo_b.py). 스탯은 전부 현재 유효 축(난이도 포함 — 낚싯대는 정식 축)이라
    #   격자에 넣으면 재현 가능해 보이지만, LADDER 를 새로 짜는 대신 값을 그대로 보존한다.
    "방벽 낚싯대": "방벽 낚싯대|B|223600|320|난이도:5,도망감소:16,경험치:13|35|왕도",
    "사자 낚싯대": "사자 낚싯대|B|238100|320|난이도:2,등급업:8,크리확률:14,크기:22|36|왕도",
    "왕관 낚싯대": "왕관 낚싯대|B|253500|320|난이도:2,도망감소:17,등급업:9,행운:12|37|왕도",
    "세관 낚싯대": "세관 낚싯대|B|253500|320|난이도:3,등급업:4,크기:5,판매보너스:9,더블찬스:3,행운:7|38|왕도",
    "문서고 낚싯대": "문서고 낚싯대|B|270000|320|난이도:3,등급업:4,경험치:15,행운:6|38|왕도",
}
...
COMMON = {
    ...
    "A": [("단단한자루", 18), ("정제된갈고리", 20), ("강철심", 36), ("진주", 45),
          ("압축흑정석", 36), ("압축철광석", 6)],
    "S": [("단단한자루", 26), ("정제된갈고리", 28), ("강철심", 60), ("별빛진주", 18),
          ("바르칸조각", 36), ("바르칸핵", 2), ("압축흑정석", 48), ("압축철광석", 12),
          ("압축자수정", 2)],
}
BUILD_MAT = {
    # ★녹슨부품은 «부품 계열 고유» 라 낚싯대에서 뺀다 — 낚싯대 쪽 초반 축은 낡은 갈고리.
    "숙련형": ("낡은갈고리", "강화철괴"),
    "행운형": ("행운의구슬", "행운의매듭"),
    "크리형": ("안개수정", "자수정"),
    "상인형": ("보석", "강화에메랄드"),
...
def check(cat):
    ...
    for c in cat:
        ...
        nan = c["st"].get("난이도", 0)
        # ★2026-09-09 — 검증 범위를 FLOOR/CAP 에서 직접 읽는다. 여기에 세 번째 사본을 두면
        #   드리프트한다(구 리터럴 D3 C5 B7 A10 S12 는 어느 표와도 맞지 않았고 하한을 아예
        #   안 봐서 S 8종이 A 하한에 앉아 있는 걸 못 잡았다).
        lo, hi = FLOOR["난이도"][g], CAP["난이도"][g]
        if not (lo <= nan <= hi):
            errs.append(f"{c['name']}: 난이도 {nan} 이 {g} 범위 {lo}~{hi} 밖")
        lk = c["st"].get("행운", 0)
...
def main():
    ...
    for c in cat:
        ...
        # ★히든은 여기 넣지 않는다 — locked=false 로 만들면 조합대에 그냥 떠서 「발견」이 사라지고,
        #   더 나쁘게는 젖은 보물상자의 히든 후보 풀(randomLockedRecipe 는 locked=true 만 본다)이
        #   통째로 비어 모든 상자 레시피 보상이 폴백(상점 판매품)으로 떨어진다.
