...
SUB_BAND = {
    ...
    # 왕도 S 는 전설(Lv62~70)·심해(Lv64~68)보다 먼저 열린다. 가격은 A 밴드를 그대로 쓴다 —
    # 재료(요구 캐스트)를 안 올린 승격이라 값만 2배로 뛰면 사다리가 깨진다(2026-09-06 유저 결정).
    ("왕도", "S"):      ((54, 61), (1640000, 1940000)),
}
...
HYBRIDS = [
    ...
    # ★2026-09-06 S 승격 — 2026-08-02 까지 왕도에 있던 S 라인의 복원이다(patch_grade_promote_s.py).
    #   08-03 생성기 도입이 이 5종을 A 로 강등하면서 «마을에서 살 수 있는 최고 등급 = A» 가 됐고
    #   S 는 히든-전설·심해에만 남아 사라진 등급이 됐다. 상점 천장은 PartShopGui.shopCeilingBlocks
    #   가 «마을 출처 S» 를 예외로 통과시킨다 — 등급만 올리면 상점·레시피 양쪽이 막힌다.
    ("왕실 작살",      "행운형", "크리형", "S", "왕도", "왕도"),
    ("근위 작살",      "공격형", "크리형", "S", "왕도", "왕도"),
    ("왕도 상회 작살", "호흡형", "행운형", "S", "왕도", "왕도"),
    ("왕립 서고 작살", "크리형", "호흡형", "S", "왕도", "왕도"),
    ("왕립 순찰 작살", "속도형", "호흡형", "S", "왕도", "왕도"),
...
LEGACY_RECIPES = [
    # ★수량은 «기준 수량»(할인 전)이다 — RecipeLoader.normalizeEquipmentRecipeMaterials 가 로드 때
    #   가공재와 DISCOUNT_KEEP_MATERIALS(진주 등)를 ÷4 한다. 인게임 표시치의 4배를 적을 것.
    #   강화철괴 48→12 · 진주 96→24 (2026-08-26 너프: 강철 작살이 너무 쌌다).
    ("HP90", "강철 작살", "",       [("단단한자루", 10), ("강철심", 18), ("강화철괴", 48), ("진주", 96)]),
...
# ══════════════════════════════════════════════════════════════════════════
COMMON = {
    "D": [("단단한자루", 3), ("강철심", 2), ("거대비늘", 2), ("물고기비늘", 6)],
    "C": [("단단한자루", 5), ("강철심", 6), ("거대비늘", 4), ("물고기비늘", 12), ("진주", 4)],
    "B": [("단단한자루", 8), ("강철심", 14), ("거대비늘", 8), ("진주", 12), ("압축흑정석", 5)],
    "A": [("단단한자루", 16), ("강철심", 26), ("거대비늘", 16), ("진주", 26),
          ("압축흑정석", 18), ("압축철광석", 5)],
    "S": [("단단한자루", 26), ("강철심", 50), ("거대비늘", 28), ("별빛진주", 14),
          ("바르칸조각", 30), ("바르칸핵", 1), ("압축흑정석", 36), ("압축철광석", 10),
          ("압축자수정", 2)],
}
