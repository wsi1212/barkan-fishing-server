...
────────────────────────────────────────────────────────────────────────────
  ...
  ★★난이도 스탯 — 구 "낚싯대 전용" 원칙 폐기, 5부품에도 신설 분산(유저 지시: "최종 G를
    ...
    전 타입에 추가(E0 D1 C2 B3 A4). 근거(minigame_sim.py Monte Carlo, 반응250ms+핑40ms):
      로드 단독 최대(기존, 실측) — 로드 자체 5(TYPE_CAP상 S=8이지만 S급 로드 둘 다 "숙련형"을
        안 써서 실제로 8에 닿는 아이템이 없다, 전부 FLOOR값 5에 머묾) + 강화 실측 최대 1
        (enhance.json 80개 로드 전수스캔 — 4가 아니었다) = rodBonus 6 → G등급 성공률 0.0%
      로드6 + 부품5종 전부 A급 숙련형(사막마을 극단형 배율로 개당 5, 5×5=25) = 총 rodBonus **31**
        → G등급 성공률 **≈2.4%**(rb=31 구간, minigame_sim.py 실측)
    ★2026-08-05 두 차례 정정 이력: 처음엔 "로드12(=8+강화4)"를 코드 주석만 믿고 검증 없이 썼다가
    37→71%로 잘못 계산했다. balance-audit 완전점검 중 실제 parts.json+enhance.json을 스캔해서
    로드 실측 최대가 5+1=6뿐임을 확인 → 31/2.4%로 정정. 교훈: "설계 의도"와 "실제 카탈로그에
    그 의도를 구현한 아이템이 존재하는가"는 별개이니 항상 라이브 데이터로 재확인할 것.
    "온갖 걸 다 써야"는 만족(부품 5개 전부 다른 빌드 대신 숙련형을 골라야 함=다른 정체성 스탯을
    전부 포기하는 트레이드오프). 2.4%는 "겨우 잡을만한"에 오히려 부합(0%→불가능이 실재하는 확률로
    바뀜). 더 후하게(예: 원래 목표였던 ~71%) 만들려면 ①S급 로드 하나를 숙련형 빌드로 바꾸거나
    ②강화표의 난이도 레벨당 배정을 올리는 별도 설계 결정이 필요 — 이번엔 수치만 정정, 결정은 보류.
    ★부품엔 강화(EnhanceManager)가 없다(강화는 "낚싯대별"·부품엔 미적용) — 위 31이 진짜 최댓값.
