...
────────────────────────────────────────────────────────────────────────────
  ...
  ★★★**2026-09-09 위 철회는 폐기됐다 — 부품에 난이도를 넣는다.** 위 문단은 08-05 이후 사실과
    달랐다: 라이브 parts.json 엔 난이도가 **28종에 박혀 있었다**(전부 이 생성기 밖 손편집).
    억센(D)·단련된(C)·방벽/성벽(B 왕도)·선단(A 상단) = «숙련형 시리즈»가 실재하고, 값도
    시리즈 안에서 어긋나 있었다(선단 = 릴3·미끼3·찌3·바늘2·줄2). 유저 결정으로 아래 사다리로
    통일했다 — balance.md §8.7 참조:
        숙련형 시리즈 난이도  D=1  C=1  B=2  A=2      (A 상한 2 = 유저 결정)
        타입별 기본 상점 라인(나무/철제/전술 릴 등)의 난이도 1 은 유지
    D/C 가 같은 값인 건 낚싯대 FLOOR(D1 C1)와 같은 형태다 — 난이도는 B 부터 갈라진다.
    ★이 생성기는 아직 난이도를 **내보내지 않는다**. LADDER 가 라이브보다 뒤처져 하드 실패하는
      상태(«카탈로그에 없는 기존 부품: 릴/수습 릴»)라 재생성 자체가 막혀 있어, 09-09 변경은
      parts.json 직접 패치로 넣었다. **LADDER 를 라이브와 동기화하는 사람이 위 사다리를 이
      생성기에 구현해야 한다** — 안 하면 08-05 처럼 재생성에서 또 사라진다.
    ★POWER_W 에도 난이도 항목이 없어 선단·방벽 시리즈는 게임 최고가치 스탯을 가격에 0 으로
      반영해 팔고 있다(선단 미끼 6,462원에 난이도 2). POWER_W 추가 + price_ladder.py 재실행이
      남은 일이다.
...
SUB_BAND = {
    ...
    # ★2026-08-31 재조정(유저 지시) — 사막 메인퀘 3-9/3-10(사막05/05b, 필요레벨18/19)이
    #   「사막 릴」을 만들라는데 구 밴드(28~34)라 착용이 십수시간 뒤였다(prod 실측, SEXY___YAMA
    #   사례: 클린 Lv23·13.5h째도 26 미도달). 퀘스트 레벨대에 맞춰 초반으로 당김.
    #   ★가격은 완전열등 회피용(전술 릴과 동가 이하면 전술 릴이 죽는다) — 라이브엔 아직
    #   레벨만 반영했다(전체 재생성 시 라이브값과 다른 44종이 「이름 유지 원칙」으로 막혀서,
    #   parts.json 을 이 파일로 통째로 재생성하지 못했다 — 그 44종부터 PRESERVE_PARTS/개명
    #   정리가 먼저 필요). 다음에 전체 재생성이 가능해지면 가격도 이 밴드로 맞춰질 것.
    ("사막마을", "B"): ((20, 26), (75000, 100000)),
...
# ★난이도 — 라이브 부품 28종엔 있으나(숙련형 시리즈, 2026-09-09 사다리 D1 C1 B2 A2)
#   이 생성기는 아직 내보내지 않아 POWER_W 에도 항목이 없다 → 선단·방벽이 **가격에 0으로
#   반영**된 상태다. 위 doc «2026-09-09» 문단 참조 — LADDER 동기화와 함께 처리할 일.
...
PRESERVE_PARTS = {
    ...
    # ★격자 밖 잔존 1종 — 2026-08-31 실측으로 발견(사막마을 B 밴드 조정 중 「이름 유지 원칙
    #   위반」으로 멈춰 있었다). 위 「수습 릴」과 같은 사정 — 격자에 없어 생성기가 소유하지
    #   않는데도 라이브엔 있다. 스탯은 그대로 보존.
    ("릴", "장터 릴"): "장터 릴|D|10000|70|경험치:7,판매보너스:5,더블찬스:3,행운:2|6|스폰마을",
    ...
    # ★2026-08-31 격자 밖 잔존 44종 일괄 등록(부품).
    #   ★정정(최초 커밋 당시 "2026-08-05 폐기된 구 숙련형 잔재"로 잘못 기록했던 걸 바로잡음):
    #   전부 2026-08-27 커밋(e65fad27·2dcdf51b — patch_wangdo_b.py/patch_line_fill.py)에서
    #   만들어진 **현재도 유효한** 콘텐츠다. 「난이도」 보유 계열(방벽·성벽·선단·억센·단련된)은
    #   그 두 스크립트가 도입한 6번째 빌드 라인 "숙련형"(난이도+도망감소, D~A 등급 부품 전체에
    #   HOME 표로 배치)인데, **이 생성기(gen_part_builds.py)의 BUILDS 딕셔너리엔 숙련형이 없다**
    #   — 그래서 "모르는 이름"으로 걸린다. 나머지(수습·장터·문서고·왕관·사자·행운의 계열)도
    #   같은 두 스크립트가 "라인 구멍"을 자동 스캔해 채운 것(village_scan 라벨러). 즉 부품에
    #   난이도가 없다는 옛 §8.7 결론은 **2026-08-27에 조용히 뒤집혔고 이 파일만 못 따라갔다.**
    #   진짜 권위는 patch_wangdo_b.py/patch_line_fill.py — 값을 바꾸려면 저길 고쳐 재실행할 것.
    #   여기서는 그 값을 그대로 보존해 이 생성기가 최소한 «모르는 이름»으로 멈추지 않게만 한다.
    ("릴", "단련된 릴"): "단련된 릴|C|41200|130|난이도:2,도망감소:26,경험치:15,행운:3|16|스폰마을",
    ("릴", "방벽 릴"): "방벽 릴|B|106500|220|난이도:2,도망감소:24,경험치:45,행운:10|36|왕도",
    ("릴", "사자 릴"): "사자 릴|B|120700|220|크리확률:12,크기:8,경험치:30,행운:4|35|왕도",
    ("릴", "선단 릴"): "선단 릴|A|817600|340|난이도:3,도망감소:72,경험치:40,행운:6|50|상단마을",
    ("릴", "억센 릴"): "억센 릴|D|26900|70|난이도:2,도망감소:26,경험치:7,행운:2|9|스폰마을",
    ("릴", "예민한 릴"): "예민한 릴|D|26900|70|크리배율:1,크리확률:9,크기:4,경험치:7,행운:2|7|스폰마을",
    ("릴", "왕관 릴"): "왕관 릴|B|128600|220|등급업:13,경험치:45,행운:18|38|왕도",
    ("릴", "행운의 릴"): "행운의 릴|C|17000|130|등급업:3,경험치:17,행운:4|13|스폰마을",
    ("미끼", "단련된 미끼"): "단련된 미끼|C|13312|130|행운:7,난이도:2,도망감소:24|17|스폰마을",
    ("미끼", "문서고 미끼"): "문서고 미끼|B|25320|220|행운:12,경험치:15,트리플찬스:2|38|왕도",
    ("미끼", "방벽 미끼"): "방벽 미끼|B|22788|220|행운:12,난이도:2,도망감소:28|37|왕도",
    ("미끼", "사자 미끼"): "사자 미끼|B|16205|220|행운:12,크리확률:12,크기:8|36|왕도",
    ("미끼", "서기 미끼"): "서기 미끼|A|24277|340|행운:17,경험치:12,트리플찬스:2|49|상단마을",
    ("미끼", "선단 미끼"): "선단 미끼|A|118241|340|행운:17,난이도:3,도망감소:67|52|상단마을",
    ("미끼", "수습 미끼"): "수습 미끼|D|3626|70|경험치:5,트리플찬스:1,행운:4|3|스폰마을",
    ("미끼", "억센 미끼"): "억센 미끼|D|5604|70|행운:4,난이도:2,도망감소:24|9|스폰마을",
    ("미끼", "예민한 미끼"): "예민한 미끼|D|4615|70|행운:4,크리확률:9,크리배율:1,크기:4|8|스폰마을",
    ("미끼", "장터 미끼"): "장터 미끼|D|4285|70|판매보너스:5,더블찬스:3,행운:4|6|스폰마을",
    ("바늘", "문서고 바늘"): "문서고 바늘|B|120700|220|크리배율:4,크리확률:9,경험치:14,트리플찬스:2,행운:4|38|왕도",
    ("바늘", "방벽 바늘"): "방벽 바늘|B|106500|220|난이도:2,크리배율:4,크리확률:24,행운:4|37|왕도",
    ("바늘", "서기 바늘"): "서기 바늘|A|789400|340|크리배율:4,크리확률:16,경험치:10,트리플찬스:2,행운:6|50|상단마을",
    ("바늘", "선단 바늘"): "선단 바늘|A|633000|340|난이도:2,크리배율:10,크리확률:33,행운:15|47|상단마을",
    ("바늘", "성벽 바늘"): "성벽 바늘|B|113600|220|난이도:2,크리배율:4,크리확률:17,행운:4|36|왕도",
    ("바늘", "수습 바늘"): "수습 바늘|D|9000|70|크리배율:2,크리확률:3,경험치:4,트리플찬스:1,행운:2|4|스폰마을",
    ("바늘", "예민한 바늘"): "예민한 바늘|D|9500|70|크리배율:3,크리확률:9,크기:4,행운:2|7|스폰마을",
    ("바늘", "왕관 바늘"): "왕관 바늘|B|128600|220|크리배율:5,등급업:9,크리확률:12,행운:16|39|왕도",
    ("바늘", "장터 바늘"): "장터 바늘|D|10000|70|크리배율:2,크리확률:3,판매보너스:5,더블찬스:3,행운:2|8|스폰마을",
    ("바늘", "행운의 바늘"): "행운의 바늘|C|29400|130|크리배율:3,등급업:4,크리확률:8,행운:4|14|스폰마을",
    ("줄", "길조 줄"): "길조 줄|C|15000|130|도망감소:13,등급업:4,행운:4|13|스폰마을",
    ("줄", "문서고 줄"): "문서고 줄|B|106500|220|도망감소:13,경험치:15,트리플찬스:2,행운:4|38|왕도",
    ("줄", "방벽 줄"): "방벽 줄|B|113400|220|난이도:2,도망감소:24,행운:18|37|왕도",
    ("줄", "사자 줄"): "사자 줄|B|128600|220|도망감소:13,크리확률:12,크기:8,행운:4|35|왕도",
    ("줄", "선단 줄"): "선단 줄|A|630300|340|난이도:2,도망감소:44,행운:33|47|상단마을",
    ("줄", "수습 줄"): "수습 줄|D|9000|70|도망감소:5,경험치:3,트리플찬스:1,행운:1|4|스폰마을",
    ("줄", "예민한 줄"): "예민한 줄|D|9500|70|도망감소:5,크리배율:1,크리확률:6,크기:3,행운:2|8|스폰마을",
    ("줄", "장터 줄"): "장터 줄|D|10000|70|도망감소:5,판매보너스:4,더블찬스:3,행운:2|5|스폰마을",
    ("찌", "단련된 찌"): "단련된 찌|C|30200|130|난이도:2,등급업:5,행운:3|16|스폰마을",
    ("찌", "문서고 찌"): "문서고 찌|B|120700|220|등급업:13,경험치:36,트리플찬스:3,행운:11|39|왕도",
    ("찌", "방벽 찌"): "방벽 찌|B|106500|220|난이도:2,등급업:12,행운:7|37|왕도",
    ("찌", "사자 찌"): "사자 찌|B|128600|220|등급업:9,크리확률:18,크기:9,행운:6|36|왕도",
    ("찌", "선단 찌"): "선단 찌|A|818900|340|난이도:3,등급업:13,행운:8|50|상단마을",
    ("찌", "섬세한 찌"): "섬세한 찌|D|9200|70|크리배율:1,등급업:2,크리확률:9,크기:4,행운:2|7|스폰마을",
    ("찌", "수습 찌"): "수습 찌|D|9000|70|등급업:2,경험치:4,트리플찬스:1,행운:2|5|스폰마을",
    ("찌", "장터 찌"): "장터 찌|D|10000|70|등급업:2,판매보너스:5,더블찬스:3,행운:2|6|스폰마을",
}
