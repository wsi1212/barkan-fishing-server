...
────────────────────────────────────────────────────────────────────────────
  §7  등급 가격대  D 200~600 / C 1,000~2,500 / B 4,000~10,000 / A 15,000~44,000
  §17 필요 레벨    E1 D5 C10 B20 A40 ← 하한. 같은 등급 안에서는 성능 점수로 갈린다.
  §8.2~8.6 타입별 <b>주스탯</b>과 범위:
      릴=도망감소(E3 D8 C10~15 B18~22 A25~30) · 줄=크리배율(E1 D2 C3 B4 A5)
      바늘=등급업(E0 D3 C5~6 B8~10 A12~15) · 미끼=경험치(E0 D10 C15~20 B25~35 A40~50)
      찌=크기(E0 D3 C5~6 B8~10 A12~15)
      행운(부품 공통 부스탯) = C1~2 B2~4 A3~6  ← 라이브 실측은 C4 B6 A8~12라 실측을 따른다
  §8.1 내구보존은 <b>낚싯대 전용</b> — 부품엔 절대 넣지 않는다.
...
TYPES = ["릴", "줄", "바늘", "미끼", "찌"]
TYPE_PRIMARY = {"릴": "도망감소", "줄": "크리배율", "바늘": "등급업", "미끼": "경험치", "찌": "크기"}

GRADE_LEVEL = {"E": 1, "D": 5, "C": 10, "B": 20, "A": 40}
LEVEL_BAND = {"D": (5, 9), "C": (10, 19), "B": (20, 34), "A": (40, 52)}
PRICE_BAND = {"D": (250, 600), "C": (1000, 2500), "B": (4000, 10000), "A": (15000, 25000)}
SUB_BAND = {
    ("스폰마을", "B"): ((20, 27), (4000, 7000)),
    ("사막마을", "B"): ((28, 34), (7000, 10000)),
    ("사막마을", "A"): ((40, 44), (15000, 18500)),
    ("상단마을", "A"): ((44, 49), (18500, 23000)),
    ("왕도", "A"):     ((49, 52), (23000, 25000)),
}
# 미끼는 소모품이라 가격 단위가 다르다(개당). 위 밴드에 이 배수를 곱한다.
BAIT_PRICE_MULT = 0.022
...
# 빌드 부스탯 (타입 무관)
SUB_VAL = {
    "행운":       {"E": 2, "D": 4, "C": 6,  "B": 9,  "A": 12},   # ★행운형 주력치
...
POWER_W = {"도망감소": 0.05, "크리배율": 2.5, "등급업": 1.0, "경험치": 0.45, "크기": 0.65,
           "행운": 0.65, "크리확률": 0.4, "판매보너스": 1.0, "더블찬스": 1.0, "난이도": 1.5}
...
def check(cat):
    ...
    for c in cat:
        ...
        if c["st"].get("행운", 0) > LUCK_CAP[g]:
            errs.append(f"{c['name']}: 행운 {c['st']['행운']} > 상한 {LUCK_CAP[g]}")
        if "내구보존" in c["st"]:
