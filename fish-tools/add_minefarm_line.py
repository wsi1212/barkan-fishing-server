...
■ 난이도 곡선 (add_quest_difficulty.py 산정 기준)
      ...
      03 특수 밀 6개 제출                   →  5칸
      04 밭 400칸 + 당근 8                 →  8칸
      05 밭 800칸 + 감자 12 + 토마토 8      → 10칸
      06 밭 2,000칸 + 버섯 12 + 방문 25    → 12칸
      07 밭 3,000칸 + 수박 8 + 방문 40     → 15칸  ★캡스톤

  ★02와 03이 동률(5칸)인 건 의도다. 03은 「더 힘든 일」이 아니라 **제출이라는 다른 동작**을
    가르친다 — 02에서 거둔 9개 중 6개를 그대로 내면 된다. 검증은 「안 내려감」만 본다.

■ 수량은 작물 실데이터에서 나온다 (감으로 적지 않았다)
  특수작물은 **한 포기당 산출이 정해져 있고 성장이 실시간**이다(`CropSpecs.ALL`).
      밀 3개/20분 · 당근 2개/30분 · 감자 2개/45분 · 토마토 2개/1시간
      양배추 2개/25분 · 버섯 3개/40분 · 수박 4개/**24시간**
  게다가 섬 작물 설치 한도는 Lv1에 **4포기**뿐이다(`CROP_LIMIT` 4/8/12/20/32).
  그래서 초반에 「특수 밀 32개」(11포기)를 시키면 한도부터 막힌다 — 6개(2포기)가 맞다.
  ★**거둬도 씨앗은 안 돌아온다**(순환 계열 특성이 없으면). 그래서 씨앗 보상은 다음 칸이
    요구하는 포기 수보다 넉넉히 준다:
      01 밀 8 → 02·03이 5포기 · 02 당근 8 → 04가 4포기 · 03 감자 8 → 05가 6포기
      04 토마토 8 → 05가 4포기 · 05 버섯 8 → 06이 4포기 · 06 수박 6 → 07이 2포기
...
■ 보상 — 밭이 밭을 키운다
  돈·경험치는 다른 사이드 수준으로 두고, **섬 편의**를 준다.
    ...
    · `autoplant:<횟수>`      자동심기 추가권 (**500 → 1,000 → 2,000**).
      ★발권 최소 단위가 500이다 — 100·200·300도 발권은 되지만 밭 한 판이면 동나 의미가 없다.
...
LINE = [
    dict(id="본사이드_마인팜01", lv=5, 이름="&a자네, 마인팜이라고 아는가",
         ...
         돈=1500, exp=250, 아이템="cropseed:밀:8",
         대화=(["자네 섬에 흙이 있지? 괭이로 갈아 보게. 서른두 칸이면 충분하네.",
               "물을 가까이 대 두게. 안 그러면 도로 흙이 돼 버려.",
               "밭이 생기거든 오게. 씨앗은 내가 주지."],
              ["서른두 칸이면 되네. 물 대는 걸 잊지 말고."],
              ["허허, 제법이군. 밭 냄새가 나는구먼.",
               "자, 이건 특수 밀 씨앗 여덟 알일세. 흔한 밀이 아니야.",
               "한 포기에 세 개씩 나오네. 아껴 심게 — 거두면 씨는 안 남아."])),

    dict(id="본사이드_마인팜02", lv=6, 이름="&a첫 수확",
         목표=["harvest|밀|3"],
         설명=["&7심었으면 거두는 게 순서지요.",
              "&f특수 밀&7을 &f3번&7 수확하세요. &8(한 번에 3개씩)",
              "&8특수 작물은 실시간으로 자랍니다 — 밀은 20분."],
         돈=3000, exp=500, 아이템="crop:밀:6,cropseed:당근:8",
         대화=(["씨앗은 줬으니 이제 자네 차례일세. 밭에 우클릭해서 심게.",
               "특수 밀은 스무 날쯤... 아니, 스무 '분'쯤 걸리네. 요즘 말로.",
               "세 포기만 거둬 보게. 그러면 감이 올 걸세."],
              ["기다리는 게 일이라네. 그동안 물고기라도 잡으러 가게."],
              ["봤나? 흙은 손댄 만큼 돌려준다니까.",
               "당근 씨앗도 가져가게. 밀만 심으면 밭이 심심하지.",
               "당근은 서른 분에 두 개일세. 밀보다 느리고 덜 나와."])),

    dict(id="본사이드_마인팜03", lv=8, 이름="&a제 손으로 기른 것",
         목표=["submitmat|작물_밀|6"],
         설명=["&7오스발트: \"장에 내다 팔 게 아니라, 나한테 가져와 보게.\"",
              "&f특수 밀 6개&7를 오스발트에게 제출하세요. &8(두 포기 분량)",
              "&8제출하면 인벤토리에서 실제로 회수됩니다."],
         돈=8000, exp=1200, 아이템="autoplant:500,cropseed:감자:8",
         대화=(["자네가 기른 걸 내 눈으로 보고 싶네. 여섯 개면 되겠어.",
               "사다가 채우려 해도 소용없어 — 특수 밀은 파는 데가 없거든. 허허.",
               "다 모으거든 들고 오게."],
              ["여섯 개일세. 두 포기면 나오지."],
              ["좋아, 좋아. 알이 굵구먼.",
               "이건 자동심기 추가권 오백 회일세. 거두면 알아서 다시 심어 줘.",
               "손이 줄면 밭을 늘릴 수 있지. 그게 시작이야.",
               "감자 씨앗도 넣었네. 마흔다섯 분짜리라 성질 급하면 못 키워."])),

    dict(id="본사이드_마인팜04", lv=11, 이름="&a밭은 손이 아니라 머리로",
         목표=["farmland|400", "submitmat|작물_당근|8"],
         설명=["&7\"서른두 칸으로는 평생 그 자리일세.\"",
              "&f경작지 400칸&7을 확보하고 &f특수 당근 8개&7를 제출하세요.",
              "&8섬 경계가 좁으면 &f/섬 업그레이드&8로 넓히세요.",
              "&8작물 설치 한도도 &f/섬 업그레이드&8에서 올립니다."],
         돈=18000, exp=2500, 아이템="fly:10:2,cropseed:토마토:8",
         대화=(["사백 칸일세. 놀라지 말게, 밭이란 게 원래 그래.",
               "다 갈고 나면 걸어다니기도 힘들 걸세. 그래서 이걸 주려는 거야.",
               "아, 특수 당근 여덟 개도 가져오고. 네 포기면 되네."],
              ["사백 칸에 당근 여덟. 섬이 좁거든 경계부터 넓히게."],
              ["이 정도면 밭이라고 불러도 되겠구먼.",
               "비행 추가권일세. 밭 위를 날아다니면 일이 반으로 준다네.",
               "토마토 씨앗도 넣었어. 이건 한 시간짜리야."])),

    dict(id="본사이드_마인팜05", lv=14, 이름="&a물을 대는 법",
         목표=["farmland|800", "submitmat|작물_감자|12", "submitmat|작물_토마토|8"],
         설명=["&7\"이제 자네 밭이 나보다 크네. 그럼 값을 해야지.\"",
              "&f경작지 800칸&7 · &f특수 감자 12개&7 · &f특수 토마토 8개&7.",
              "&8감자는 45분, 토마토는 1시간. 자라는 시간이 다른 걸",
              "&8섞어 심으면 손이 노는 시간이 없습니다."],
         돈=40000, exp=5000, 아이템="autoplant:1000,cropbundle:밀:1,cropseed:버섯:8",
         대화=(["팔백 칸에, 감자 열둘, 토마토 여덟일세.",
              ...
              ["…자네, 이제 농부구먼.",
               ...
               "버섯 씨앗도 주지. 어두운 데서도 자라는 놈이야."])),

    dict(id="본사이드_마인팜06", lv=18, 이름="&6마인팜에는 사람이 온다",
         목표=["farmland|2000", "submitmat|작물_버섯|12", "islandvisit|25"],
         설명=["&7\"밭이 커지면 사람이 오네. 그게 순서야.\"",
              "&f경작지 2,000칸&7 · &f특수 버섯 12개&7 · &f섬 방문 25회&7.",
              "&8&f/섬 공개&8 설정과 워프를 열어 두면 사람이 찾아옵니다.",
              "&8★방문은 남이 올려 주는 수치입니다 — 혼자서는 못 채웁니다."],
         돈=70000, exp=8000, 아이템="fly:20:3,cropseed:수박:6,cropbundle:당근:1",
         동시진행=True,
         대화=(["이천 칸. 이제 슬슬 남들 눈에 띌 걸세.",
               "버섯도 열둘 가져오고 — 네 포기면 나오네.",
               "그리고 문을 열어 두게. 스물다섯 번만 다녀가면 되네.",
               "밭은 혼자 보면 밭이지만, 남이 보면 마을이 되거든."],
              ["이천 칸에 버섯 열둘, 방문 스물다섯 번. 사람은 억지로 못 부르니 느긋하게."],
              ["사람이 왔다 갔구먼. 발자국이 보이네.",
               "수박 씨앗 여섯 알을 주지. 하루가 꼬박 걸리는 놈이야 — 자네니까 주는 걸세."])),

    dict(id="본사이드_마인팜07", lv=22, 이름="&6바르칸의 마인팜",
         목표=["farmland|3000", "submitmat|작물_수박|8", "islandvisit|40"],
         설명=["&7오스발트: \"내가 평생 못 한 걸 자네가 하는군.\"",
              "&f경작지 3,000칸&7 · &f특수 수박 8개&7 · &f섬 방문 40회&7.",
              "&8수박은 한 포기에 꼬박 &f24시간&8입니다 — 두 포기 분량.",
         ...
         대화=(["마지막일세. 삼천 칸.",
               ...
               "그리고 수박 여덟. 그놈은 하루를 꼬박 기다려야 하네.",
               "사람도 마흔은 다녀가야 하고.",
               "밭이 그만하면 그건 이제 자네 섬이 아니라 자네 마을일세."],
              ["삼천 칸. 하루에 백 칸씩 갈아도 한 달일세. 수박은 심어 놓고 잊고 지내게."],
              ["…내가 살아서 이걸 볼 줄은 몰랐네.",
...
if any(ranks[i] < ranks[i - 1] for i in range(1, len(ranks))):
    ...
    flat = [i + 1 for i in range(1, len(ranks)) if ranks[i] == ranks[i - 1]]
    print(f"\n난이도 곡선: {' → '.join(str(r) for r in ranks)}  ✓ 안 내려감"
          + (f"  (동률 구간: {flat})" if flat else ""))
