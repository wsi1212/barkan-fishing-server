#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""스폰마을 사이드 — 「오스발트의 마인팜」 8단 라인 (2026-08-16).
...
■ 성격 — 오래 끌고 가는 배경 과제
  다른 사이드는 앉은자리에서 끝난다. 이건 **Lv5에 시작해 Lv30까지 이어지는** 장기 과제다.
...
■ 난이도 곡선 (add_quest_difficulty.py 산정 기준)
      ...
      07 밭 3,000칸 + 수박 8 + 방문 40           → 15칸   칭호 `마인팜장`
      08 밭 10,000칸 + 양배추 32 + 수박 32 + 방문 60 → 16칸  ★칭호 `대지주`
...
■ 08 「대지주」 — 만 칸은 밭이 아니라 지형이다
  ...
  즉 **한 층으로 만 칸을 채우려면 최소 반경 50(Lv2)**, 물길·통로까지 넣으면 실질
  **반경 75(Lv3, 400,000원)**다. 층층이 쌓아도 된다 — 스캐너는 섬 region의 Y 전 구간을
  훑으므로 **높이를 가리지 않는다.** 마인팜의 본질이 다층 밭이라 일부러 그렇게 짰다.

  ★방문 60은 07의 40에서 20을 더한 것이다. `visitCount`는 섬이 생긴 순간부터 누적이고
    리셋이 없어서, 07을 깰 무렵이면 이미 상당히 차 있다. 08은 `동시진행`이라 붙잡고 있어도
    아무것도 안 막는다.
...
■ 수량은 작물 실데이터에서 나온다 (감으로 적지 않았다)
  ...
  ★**거둬도 씨앗은 안 돌아온다**(순환 계열 특성이 없으면). 그래서 씨앗 보상은 다음 칸이
    요구하는 포기 수보다 넉넉히 준다:
      ...
      04 토마토 8 → 05가 4포기 · 05 버섯 8 → 06이 4포기
      06 수박 6 + 07 수박 10 → 07이 2포기 · 08이 8포기 (총 16, 여유 6)
      07 양배추 20 → 08이 16포기 (7종 중 마지막으로 문이 열리는 작물)
...
# ══ 칭호 ═══════════════════════════════════════════════════════════════════
NEW_TITLES = {
    "마인팜장": {"name": "&2마인팜장", "color": "&2",
               "desc": "&7섬을 통째로 밭으로 만든 사람", "icon": "hay block"},
    # 대지주 = 만 칸. 마인팜장(3천)의 세 배가 넘는 자리라 색도 한 단 올린다.
    "대지주": {"name": "&5대지주", "color": "&5",
             "desc": "&7섬 만 칸을 밭으로 갈아엎은 사람", "icon": "grass block"},
}
for _tid, _tdef in NEW_TITLES.items():
    if _tid not in TITLES:
        TITLES[_tid] = _tdef
        if "order" in T and _tid not in T["order"]:
            T["order"].append(_tid)
...
LINE = [
    ...
    dict(id="본사이드_마인팜07", lv=22, 이름="&6바르칸의 마인팜",
         ...
         아이템="autoplant:2000,fly:20:5,cropbundle:밀:2,cropbundle:토마토:2,"
               "cropseed:양배추:20,cropseed:수박:10",
         칭호="마인팜장", 동시진행=True,
         대화=(["마지막일세. 삼천 칸.",
              ...
              ["…내가 살아서 이걸 볼 줄은 몰랐네.",
               ...
               "…양배추랑 수박 씨앗을 좀 넣어 뒀네. 왜냐고?",
               "허허. 아직 한 가지가 남았거든. 몸이 좀 더 자라거든 오게."])),

    dict(id="본사이드_마인팜08", lv=30, 이름="&5대지주",
         목표=["farmland|10000", "submitmat|작물_양배추|32",
              "submitmat|작물_수박|32", "islandvisit|60"],
         설명=["&7오스발트: \"만 칸일세. 농담 아니야.\"",
              "&f경작지 10,000칸&7 · &f특수 양배추 32개&7 · &f특수 수박 32개&7 ·",
              "&f섬 방문 60회&7.",
              "&8한 층으로는 &f섬 경계 Lv3(반경 75)&8쯤은 돼야 물길·통로까지 들어갑니다.",
              "&8층층이 쌓아도 됩니다 — 경작지는 높이를 가리지 않고 셉니다.",
              "&8수박은 한 포기 24시간 · 여덟 포기 분량. 미리 심어 두세요.",
              "&8완료 시 칭호 &5대지주&8를 받습니다."],
         돈=1000000, exp=60000,
         아이템="autoplant:2000:3,fly:20:10,cropbundle:수박:2,cropbundle:버섯:2,"
               "cropbundle:감자:2,cropbundle:양배추:2",
         칭호="대지주", 동시진행=True,
         대화=(["만 칸일세. 농담 아니야.",
               "삼천은 밭이지만 만은 지형이야. 위로 쌓아도 되네 — 흙은 높이를 안 가려.",
               "그리고 양배추 서른둘, 수박 서른둘. 수박은 미리 심어 두게, 하루짜리니까.",
               "이걸 해내면 자네는 더 이상 농부가 아닐세. 땅 주인이지."],
              ["만 칸. 세다가 헷갈리면 그냥 계속 갈게. 내가 세어 줄 테니."],
              ["…말이 안 나오는군.",
               "내 평생 이런 밭은 본 적이 없네. 남의 것으로도, 내 것으로도.",
               "대지주. 그렇게 부르겠네. 이제 자네한테 가르칠 게 없어.",
               "…가끔 들르게. 늙은이 밭 구경이나 하러."])),
]
