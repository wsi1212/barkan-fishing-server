#!/usr/bin/env bash
# Prepare an inactive local macOS dev proxy network. Does not stop or reconfigure main Paper.
set -euo pipefail
...
PAPER_JAR=${BARKAN_DEV_PAPER_JAR:-/Users/user/Library/Application Support/minecraft/libraries/java/paper-1.21.11-132.jar}
...
GEYSER_URL=https://download.geysermc.org/v2/projects/geyser/versions/2.11.3/builds/1245/downloads/velocity
GEYSER_SHA256=306abe38291d24e874c7286e1c04b537ac3ab5e3901b710f56bd242500bedfbf
...
install -d \
  ...
  "$NETWORK_ROOT/bin" "$NETWORK_ROOT/logs"
...
download_verified(){
  ...
  [ "$(shasum -a 256 "$temporary" | awk '{print $1}')" = "$expected" ] || {
    echo "SHA-256 불일치: $destination" >&2; return 1; }
...
install -m 0755 "$SCRIPT_DIR/dev-network.sh" "$NETWORK_ROOT/bin/dev-network.sh"
install -m 0755 "$SCRIPT_DIR/configure-backend.py" "$NETWORK_ROOT/bin/configure-backend.py"

echo "비활성 dev network 준비 완료: $NETWORK_ROOT"
