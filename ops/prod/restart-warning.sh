#!/usr/bin/env bash
# =====================================================================
# 바르칸 prod 데일리 점검(06:00 KST) 사전예고 — 30/10/5/1분 전 각각 cron으로 호출.
# 점검은 재시작하지 않는다. 이 파일은 접속자에게 점검 일정을 안내하는 역할만 한다.
...
"$DIR/rcon.py" "say [서버] ${MINUTES}분 후 정기 점검입니다. 재시작 없이 저장·상태만 확인하며 플레이는 계속할 수 있습니다." >/dev/null 2>&1
