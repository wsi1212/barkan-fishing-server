...
### prod flatroom 에 놓인 것들 (region 투하)

| id | 이름 | 범위 x / y / z | 보기 좌표 |
|----|------|----------------|-----------|
| 17 | 아르타잔의 성역 | 5000~5447 / 100~224 / 5000~5575 | `/tp 5220 250 5280` |
| 07 | 아틀란티스 로비 | 6700~6850 / 100~175 / 5000~5150 | `/tp 6775 200 5075` |
| 13 | 오션 하트랜드 | 6700~6908 / 100~267 / 5300~5508 | `/tp 6800 290 5400` |

`place_more.py` 의 `JOBS` 에 (id, x, y, z) 를 추가하면 같은 방식으로 더 놓을 수 있다.

- ★**빈 region 을 고를 때 «전에 비어 있었다» 를 믿지 말 것.** 유저가 그 근처를 한 번 날아다니면
  평지가 생성되면서 region 파일이 만들어진다. 2026-09-04 에 17번을 넣은 뒤 x/z region 8~12 가
  전부 생겨서, 07·13 은 region 13 (x 6656~7167) 으로 밀어야 했다. 매번 `ls region/` 로 실측.
- ★**13 은 원본의 «바닷물» 이 파랑/청록 유리·양털이라 그걸 지우면 섬과 물고기만 공중에 뜬다**
  (209×209×168 에 블록 34만개 = 대부분 빈 공간). 잘못 놓인 게 아니다 — 좌표를 몇 개 찍어 보고
  «비었다» 고 판단하면 오진한다. 실제 범위는 `patch2.json` 과 위 표.
