...
#        편집은 dev에서 하고 배포할 것.
DATA_FILES=("npc.json" "dialogue.json" "titles.json" "parts.json" "enhance.json" "recipes.json" "materials.json" "fragment-economy.json" "quests.json" "fish.json" "item-flavor.json" "cashshop.json")
...
if [ "$RESTART_PROD" = 0 ]; then
  ...
  if ! ssh -i "$SSH_KEY" -o StrictHostKeyChecking=no \
    ...
    "set -e
     ...
     for f in npc.json dialogue.json titles.json parts.json enhance.json recipes.json materials.json fragment-economy.json quests.json fish.json item-flavor.json cashshop.json; do
