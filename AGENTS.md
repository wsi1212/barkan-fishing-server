...
- **prod 홈 전체 재귀 검색 금지 (2026-09-18)** — prod SSH 안에서 `/home/ubuntu`·`$HOME`·`~` 전체를 재귀 `grep`/`rg`하거나 깊은(3단계 이상)/무제한 `find`를 돌리지 않는다. 실제 사고에서 한 점검 세션이 `grep -R … /home/ubuntu`로 21GB를 읽어 볼륨 I/O 84%, 청크 읽기 지연 75~134ms, TPS 급락을 만들었다. 반드시 `~/mcserver/logs`, `~/mcserver/scripts`처럼 대상 디렉터리를 좁힌다. `ops/hooks/guard-prod-recursive-search.py`가 Claude·Codex 양쪽 Bash PreToolUse에서 강제하며 `guard-prod-recursive-search-selftest.py`로 검산한다.
