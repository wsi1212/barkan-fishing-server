...
### 외곽선 판정 규칙 (2026-08-19)
- 아이템 텍스처의 **원본 해상도 기준**으로 판단한다. 16×16 다운샘플, 슬롯 목업, 인게임 축소 화면을 기준으로 테두리 두께를 재단하지 않는다.
- 모든 아이템은 원본 텍스처에서 검은 외곽선 **정확히 1px**을 유지한다.
- 수정할 때는 기존 검은 테두리를 먼저 모두 제거한 뒤, 실루엣 바깥에 원본 해상도 기준 1px 검정선을 다시 생성한다.
- 테두리 바깥에 번진 검정 헤일로만 제거하며, 내부 검정 디테일·하이라이트·투명도는 보존한다.
