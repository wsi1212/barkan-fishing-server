...
# ★조합 2x2 + 결과 칸을 '넓히는' 시도는 접었다(2026-08-11).
#   구멍이 아이콘 상자보다 5~7px 작아 아이콘이 테두리를 덮는데, 중심은 1.5px 안쪽으로
#   맞아 있다. 넓히려면 액자를 바깥으로 밀어야 하고 2x2 는 가운데 구분선을 공유해서
#   그걸 깎아먹는다 — 같은 수법으로 분해창 초록판을 파먹은 직후라 안 한다.
#   제대로 맞추려면 구멍 64px 로 다시 받아야 한다(발주서에 그 규격을 박아 뒀다).
