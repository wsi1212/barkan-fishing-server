...
HUBS = {
    ...
    # 길드 허브 — 기존 /길드 메뉴는 버튼 9개가 54칸에 흩어져 있어 뭐가 중요한지 안 읽혔다.
    # 자주 쓰는 여섯을 타일로 올리고, 나머지(설정·신청·둘러보기·해산)는 아래 아이콘 줄로 내린다.
    # ★엠블럼은 지금 만들어지는 중인 기능(GuildThumbnailService)이라 자리를 미리 잡아 둔다.
    "guild": ("길드", [
        ("길드 섬", 0, 1, 3, 2), ("업그레이드", 3, 1, 3, 2), ("기부", 6, 1, 3, 2),
