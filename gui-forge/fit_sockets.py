...
PROBE = PAD                         # 아이콘 상자 바깥 여백(4px) 안에서만 본다
NO_SOCKET = PAD                     # 여백을 다 훑어도 경계가 없으면 그 칸엔 액자가 없다
# ★상한은 PAD(4) 다. 칸 경계를 넘어 떠오게 풀어 봤더니(5~9px) 분해창 초록판처럼
#   칸이 이어진 면에서 **옆 칸 바깥의 바닥을 끌어와 판을 파먹었다**(2026-08-11 실측).
#   4px 는 아이콘 상자와 셀 경계 사이 여백이라 그 안에서만 움직이면 안전하다.
#   그보다 벌어진 칸은 액자가 옆 칸과 공유하는 구분선 밖이라 그림을 다시 받아야 한다.
...
STAMP = {
    ...
    # ★분해창은 2026-08-11 에 입력칸을 행2~4 로 내렸다 — 그 세 줄은 아트에 초록판이 다 있어
    #   도장이 필요 없다. 예전 행1~3 시절 매핑을 남겨두면 엉뚱한 칸을 덮는다.
