...
SHEETS = {
    ...
    "guildupgrade": ("길드 업그레이드", [
        ...
        ("s", ""),
        ("hi", "★이 창은 위아래 두 덩어리다. 그게 전부다."),
        ("p", "위(0~17) = 길드원 버프. 길드원 전원의 **몸에** 붙는 것."),
        ("p", "아래(36~53) = 섬 업그레이드. 길드섬이라는 **땅에** 붙는 것."),
        ("p", "위는 걸리는 것(유리병·부적·깃발), 아래는 쌓이는 것(돌·흙·기둥) 정도로"),
        ("p", "재질을 갈라 주면 한눈에 읽힌다. 그림 두 장처럼 보이라는 건 아니다 -"),
        ("p", "같은 방인데 위쪽 선반과 아래쪽 바닥이 다른 정도."),
        ("s", ""),
        ("hi", "★넷째 줄(28~34) 일곱 칸이 그 경계다."),
        ("p", "지금 코드가 회색 유리판을 깔아 띠를 만들고 있다."),
        ("w", "그림으로 띠를 그려도 그 위에 유리판이 덮인다 - 홈으로 파 둘 것"),
        ("p", "양끝 세로줄까지 가지 않고 한 칸씩 들어와 끊긴다. 프레임 안쪽에"),
        ("p", "끼워 넣은 띠처럼 - 벽을 가로지르는 선반이나 몰딩."),
        ("s", ""),
        ("hi", "★셋째 줄(18~26)은 통째로 그림이다."),
        ("p", "쓰는 칸이 하나도 없다. 위 덩어리와 경계선 사이의 숨 쉬는 자리 -"),
        ("p", "여기서 '길드 회관'다움을 만들어 준다. 가장 자유로운 줄이다."),
        ("s", ""),
        ("hi", "★버프 여섯은 한 줄로 나란히 선다(10~15)."),
        ("p", "왼쪽부터 낚시 경험치 · 크기 · 등급업 · 더블확률 · 이동속도 · 성급함."),
        ("p", "순서가 코드에 박혀 있어 안 바뀐다. 여섯이 **같은 격**으로 보여야 한다 -"),
        ("p", "하나만 크거나 화려하면 그게 제일 좋은 버프인 줄 안다."),
        ("p", "칸 위에 레벨이 글자로 찍히니 홈 안쪽은 조용하게."),
        ("s", ""),
        ("hi", "★아래 일곱(46~52)은 종류가 제각각이다."),
        ("p", "인원 확장 · 섬 크기 · 호퍼 · 액자 · 가구 · 특수작물 · 워프."),
        ("p", "일곱이 한 줄에 붙어 있으니 테두리는 얇게. 두꺼우면 답답해진다."),
        ("s", ""),
        ("hi", "★맨 왼쪽·맨 오른쪽 세로줄은 통째로 프레임이다."),
        ("p", "위아래로 뚫린 두 열이라 여기서 창의 테두리가 만들어진다."),
        ("p", "홈은 그 안쪽(가운데 일곱 열)에만 있다 - 왼쪽 위 뒤로가기 하나만 예외."),
        ("p", "기둥·경첩·밧줄처럼 세로로 읽히는 것이 어울리는 자리다."),
        ("s", ""),
        ("p", "0 = 뒤로가기, 4 = 「길드원 버프」 머리, 40 = 「섬 업그레이드」 머리."),
        ("p", "머리 둘은 각 덩어리의 이름표라 살짝 도드라져도 좋다."),
        ("w", "53(오른쪽 맨 아래)은 비어 있다 - 닫기 버튼이 없는 창이다(Esc)"),
        ("p", "비워 두지 말고 그림으로 채울 것."),
    ]),
}
...
THEME_GUILD_UP = ("컨셉 - 회관 안쪽, 설비를 늘리는 방",
                  "길드 허브(회관 게시벽)에서 한 칸 들어간 방이다. 같은 어두운 나무 ·",
                  "놋쇠 · 청록을 쓰되, 게시벽이 '보는 곳'이라면 여긴 '고치는 곳'이다.",
                  "도면과 자, 벽에 걸린 설비 대장, 쌓아 둔 자재.",
                  "돈을 써서 길드가 커지는 화면이라 - 값이 나가 보이면 좋다.",
                  "★위(사람에게 붙는 버프)와 아래(땅에 붙는 섬)를 재질로 갈라 줄 것.",
                  "★칸이 스물넷이나 된다. 배경이 시끄러우면 버튼이 안 읽힌다.")
