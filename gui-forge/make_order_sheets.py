...
SHEETS = {
    ...
    "workbench": ("장비 작업대", [
        ("p", "/장비 로 여는 창. 낚싯대에 달 부품을 끼우고 빼는 자리다."),
        ("p", "세 줄뿐이라 좁다. 쓰는 칸은 위 가운데 하나와 가운데 줄 다섯뿐."),
        ("s", ""),
        ("hi", "★가운데 줄 다섯 칸이 이 창의 전부다."),
        ("p", "플레이어가 인벤토리에서 부품을 **끌어다 넣는** 칸이다."),
        ("p", "그래서 '진열장'이 아니라 '꽂는 자리'로 보여야 한다 -"),
        ("p", "비어 있을 때 넣고 싶어지는 홈. 얕은 요람, 가죽 끈 고리, 놋쇠 클립 같은."),
        ("p", "다섯은 왼쪽부터 릴·줄·바늘·미끼·찌 순서로 고정이다."),
        ("p", "칸 바깥 여백(위아래)에 그 부품을 암시하는 음각이나 낙인을 넣어도 좋다 -"),
        ("p", "★단 홈 안쪽에는 아무것도 그리지 말 것. 부품 아이콘이 그 위에 얹힌다."),
        ("p", "★넷째 칸(미끼)은 개수 숫자가 칸 오른쪽 아래에 겹쳐 찍힌다(예 99)."),
        ("p", "  거기에 밝은 장식이 오면 숫자를 못 읽는다 - 그 모서리는 어둡고 조용하게."),
        ("s", ""),
        ("hi", "★첫째 줄 가운데 하나는 장비 요약이다."),
        ("p", "지금 낚싯대와 합산 스탯을 보여주는 자리. 다섯 칸의 '주인'처럼 보이게 -"),
        ("p", "위에 걸린 낚싯대, 명패, 펼친 장비 대장 같은. 나머지 첫 줄은 그림 몫이다."),
        ("p", "닫기 버튼이 없는 창이니 오른쪽 끝도 비워 두지 말고 그림으로 채울 것."),
        ("s", ""),
        ("hi", "★셋째 줄은 통째로 그림이다."),
        ("p", "쓰는 칸이 하나도 없다. 작업대 아래 서랍·공구걸이·감아둔 낚싯줄 뭉치 등"),
        ("p", "가장 자유롭게 쓸 수 있는 줄이다. 여기서 '작업장'다움을 만들어 준다."),
        ("s", ""),
        ("p", "톤: 항해사의 장비함을 펼쳐 놓은 작업대. 기름 먹인 나무, 놋쇠, 가죽."),
        ("p", "인벤토리 판과 한 세트로 보이게 - 같은 창에서 아이템을 주고받는 화면이다."),
    ]),
    "repair": ("장비·통발 수리", [
        ("p", "/수리 로 여는 3행 전용 창. 장착 부품을 보여주고, 통발은 한 칸에 직접 넣어"),
        ("p", "수리하는 작업대다. 소지품에서 손상 통발을 자동으로 목록화하지 않는다."),
        ("s", ""),
        ("hi", "★제목 띠는 비워 두고, 아이템 5개는 둘째 줄에 둔다."),
        ("p", "장착 부품과 통발은 화면 12~16번에 한 줄로 연속 배치한다."),
        ("p", "제목 띠는 수리공의 명판·걸쇠처럼 쓰되 글자는 굽지 말 것."),
        ("s", ""),
        ("hi", "★둘째 줄 화면 12~16번은 아이템 5개를 연속 배치한다."),
        ("p", "릴·줄·바늘·찌를 먼저 놓고, 마지막 화면 16번에 통발을 직접 올린다."),
        ("p", "다섯 슬롯은 하나의 연속된 정비 트레이처럼 보이게 한다."),
        ("s", ""),
        ("hi", "★22번은 이 화면의 유일한 큰 동작이다."),
        ("p", "모두 수리 버튼이 들어갈 자리. 중앙 하단에 놋쇠 레버·붉은 핀·"),
        ("p", "달군 모루 자국처럼 눌러야 할 느낌을 주되 아이콘을 방해하지 말 것."),
        ("s", ""),
        ("p", "분위기: 강화창의 용광로보다 차분한 정비 작업대."),
        ("p", "기름 먹인 목재, 그을린 철, 낮은 주황 불빛, 물통 또는 숫돌."),
        ("p", "내구도 상태 색(초록·노랑·빨강)은 아이템 lore가 보여 주므로"),
        ("p", "배경은 초록·빨강으로 과하게 물들이지 말고 중성적인 철·청동으로 유지한다."),
    ]),
    "artifact": ("유물 감정대", [
        ("p", "사막마을 사피르의 유물 감정 화면. 오아시스에서 건진 미감정 유물을"),
        ("p", "감정료와 함께 하나씩 감정하는 3행 창이다."),
        ("s", ""),
        ("hi", "★가운데 둘째 줄 14번(코드 raw 13)이 유일한 입력·감정 자리다."),
        ("p", "점토·석판·황동으로 된 고고학 감정대. 아이콘이 올라갈 홈 안쪽은 비운다."),
        ("p", "왼쪽에는 붓·표본병, 오른쪽에는 등불·접힌 지도 같은 도구를 둔다."),
        ("s", ""),
        ("hi", "★오른쪽 아래 27번(코드 raw 26)은 닫기 버튼 자리다."),
        ("p", "감정대의 작은 황동 걸쇠처럼 읽히되, 아이콘을 가리지 않게 얕은 홈으로."),
        ("s", ""),
        ("p", "톤: 사피르의 오래된 조사실. 모래빛 사암·짙은 목재·낮은 호박색 등불."),
        ("p", "확률표 lore가 길게 올라오므로 배경에 글자를 굽지 말고 중심을 조용하게 둔다."),
    ]),
    "horse": ("말 대여소", [
        ("p", "마구간 안쪽에 마련한 말 대여 진열대. 3행 27칸 화면이다."),
        ("p", "첫째 줄 가운데는 보유 금액, 둘째 줄에는 네 등급 말을 한 칸씩 띄워 놓는다."),
        ("s", ""),
        ("hi", "★말 4종은 코드 raw 10·12·14·16에 정확히 올라간다."),
        ("p", "왼쪽부터 조랑말·갈색 여행마·백은 준마·흑요석 천리마 순서."),
        ("p", "리드·안장·철 말 갑옷·금 말 갑옷의 실제 아이템 실루엣이 주인공이다."),
        ("s", ""),
        ("hi", "★활성 대여 중일 때만 raw 22에 소환 버튼이 뜬다."),
        ("p", "그 자리는 비어 있어도 어색하지 않은 마구간 바닥/난간으로 만들고,"),
        ("p", "버튼이 뜨면 추가 비용 없는 호출 장치처럼 읽히게 한다."),
        ("s", ""),
        ("p", "톤: 기름 먹인 목재·가죽·황동, 말발굽·마구 걸이·건초 상자."),
        ("p", "가격과 능력치 lore가 아이템 아래에 붙으므로 배경에는 글자를 넣지 않는다."),
    ]),
    "questlist": ("퀘스트 도감 메인", [
        ("p", "퀘스트 도감의 실제 첫 화면. 3행 27칸에서 카테고리로 들어간다 -"),
        ("p", "메인·일일·주간·사이드 네 문서철을 고르는 표지다."),
        ("s", ""),
        ("hi", "★첫째 줄 가운데 하나는 제목, 둘째 줄 네 칸이 카테고리다."),
        ("p", "각 카테고리는 서로 다른 기록철처럼 보이되 같은 여행자 수첩 계열로 묶인다."),
        ("s", ""),
        ("p", "톤: 바르칸 열도의 여행자 수첩. 낡은 가죽과 양피지, 놋쇠 장식."),
    ]),
    "questjournal": ("퀘스트 저널", [
        ("p", "쉬프트+Q 로 여는 실제 창. 4행 36칸이다."),
        ("p", "위 두 줄은 메인 이야기, 셋째 줄은 구분 띠, 아래 한 줄은 일일 퀘스트다."),
        ("s", ""),
        ("hi", "★13=진행 중 메인 · 15=다음 이야기 · 27=일일 라벨."),
        ("p", "28~34는 내용이 바뀌는 일일 퀘스트라 안쪽은 조용하게 유지한다."),
        ("p", "18~26은 아이템으로 덮지 않고 배경의 구분 띠가 보이게 둔다."),
        ("s", ""),
        ("p", "NPC 대화 선택지 인벤토리도 36칸이지만 용도가 달라 별도 흐름으로 공유한다."),
    ]),
...
THEME_GUILD = ("컨셉 - 길드 회관의 게시벽",
               "메뉴·내 정보와 같은 타일형 화면이다. 그 둘과 한 세트로 보여야 한다 -",
               "어두운 나무 · 놋쇠 · 청록. 다만 여긴 '우리 것'이라는 자부심이 붙는다:",
               "벽에 건 현수막, 못 박은 명패, 촛대. 개인 화면(내 정보)이 책상이라면",
               "여긴 여럿이 드나드는 회관 벽이다.",
               "★타일 여섯이 나란히 선다 - 배경이 시끄러우면 버튼이 안 읽힌다.")
...
THEME_ARTIFACT = ("컨셉 - 사피르의 사암 감정실",
                  "오아시스에서 건진 유물을 조사하는 작은 고고학 작업대다.",
                  "모래빛 사암·짙은 목재·황동·낮은 호박색 등불로 사막마을의 온도를 잡는다.",
                  "중앙 감정 홈이 시선의 끝이어야 하고, 붓·표본병·지도는 가장자리에서 받친다.",
                  "★확률표 lore가 길게 붙으므로 중앙 주변은 읽기 편하게 조용히 둔다.")
THEME_HORSE = ("컨셉 - 기름 먹인 목재 마구간",
               "말을 고르고 대여하는 마구간 안쪽 진열대다.",
               "갈색 판재·가죽·황동·건초색을 쓰고, 말발굽과 마구 걸이를 좌우 장식으로 둔다.",
               "네 말 아이콘이 한 줄의 등급 진열처럼 읽혀야 하며, 아이템 lore가 주인공이다.",
               "★raw 22는 활성 대여 때만 소환 버튼이 뜨므로 평소에도 배경이 자연스러워야 한다.")

THEMES = {"iceshop": THEME_ICE,
          "guild": THEME_GUILD,
          "workbench": THEME_INV,
          "questlist": THEME_QUEST, "questjournal": THEME_QUEST, "questnpc": THEME_QUEST, "skillhub": THEME_SKILL, "skilltree": THEME_SKILL,
          "inventory": THEME_INV,
          "dexmain": THEME_DEX, "dextab": THEME_DEX,
          "dexfish": THEME_DEX, "dexisland": THEME_DEX,
          "dialogue": THEME_NPC, "npcdialog": THEME_NPC,
          "repair": THEME_SMITHY,
          "artifact": THEME_ARTIFACT, "horse": THEME_HORSE}
...
def build(name):
    ...
    # 새 전용판은 완성된 배경 자체가 가장 정확한 발주 기준이다. 기존 화면은
    # 별도의 중립 템플릿을 유지하고, 템플릿이 없는 새 화면만 source를 사용한다.
    if not os.path.exists(plate_path):
        plate_path = os.path.join(HERE, "src", name, "bg_source.png")
