#!/usr/bin/env python3
"""발주 시트 - 뼈대판 + 지시문을 한 장의 그림으로 굽는다.

발주서(.md)와 뼈대판(.png)을 따로 넘기면 **그림을 못 찾는 일이 생긴다**(2026-08-08).
그림 한 장에 지시문까지 얹어 두면 그 한 장만으로 작업이 된다.

왼쪽 = 실제 크기 뼈대판(여기에 덧칠) · 오른쪽 = 규칙과 컨셉.
★오른쪽 설명판은 **참고용이라 잘라내고 왼쪽만 쓰면 된다** - 시트에도 그렇게 적어 둔다.

사용: python3 make_order_sheets.py [이름 ...]
산출: src/<이름>/_order.png
"""
import os
import sys
import textwrap

from PIL import Image, ImageDraw, ImageFont

import make_page_layouts as L

HERE = os.path.dirname(os.path.abspath(__file__))
RP = os.path.expanduser("~/development/barkan-resourcepack")
FONT_TTF = os.path.join(RP, "assets/barkan/font/aggro_bold.ttf")
PANEL_W = 720
BG, INK, DIM, HI, WARN = (26, 26, 30), (238, 238, 242), (168, 168, 176), (255, 208, 96), (255, 130, 110)

COMMON = [
    ("t", "그리는 법"),
    ("p", "왼쪽 판을 열어서 그 위에 재질과 분위기만 입힌다."),
    ("p", "표시된 자리를 옮기지 말 것 - 이미 정확한 좌표다."),
    ("p", "★홈 하나는 72px 칸이고 아이콘은 그 가운데 64px 다. 액자 구멍을 그보다 크게"),
    ("p", "  그리면 아이콘과 테두리 사이에 틈이 남는다(확대하면 바로 보인다)."),
    ("grid", ""),            # 칸 좌표를 숫자로 박는다(grid_for)
    ("p", "★작게 그려도 안 된다 - 아이콘이 테두리 위로 올라탄다."),
    ("p", "  칸 여백은 사방 4px 뿐이다. 액자 테두리를 그보다 두껍게 그리면 이웃 칸을"),
    ("p", "  침범해 옆 액자가 잘려 보인다. 칸 사이를 떼어 놓으려 하지 말 것 -"),
    ("p", "  이 격자에서 칸들은 원래 맞닿아 격자선처럼 이어진다."),
    ("s", ""),
    ("legend", ""),          # 그 화면에 실제로 있는 역할만 자동으로 채운다
    ("s", ""),
    ("t", "어기면 게임에서 깨진다"),
    ("hard", ""),            # 화면 종류별 주의문 (아래 HARD)
    ("w", "글자 금지 - 글자는 코드가 찍는다"),
]

# 상자 창(우리가 띄우는 GUI)과 바닐라 인벤토리는 지키는 게 다르다.
HARD = {
    # ★대시는 '-' 만 쓴다. em대시(—)는 aggro_bold.ttf 에 글리프가 없어 시트에 두부(⊠)로
    #   찍힌다. 이 표는 모든 상자 시트가 공유하므로 여기 하나가 전체 발주서를 오염시킨다.
    "chest": [("w", "액자 구멍은 칸 정중앙에 정확히 64px - 크면 아이콘과 틈이 생긴다"),
              ("w", "캔버스 크기 그대로. 1px도 다르면 안 된다"),
              ("w", "★격자를 가로지르는 장식(책 접힘선·기둥·걸쇠·관)을 격자에 붙이지 말 것"),
              ("w", "  격자가 틀어지면 우리가 옮기는데, 붙어 있으면 그 장식까지 깨진다"),
              ("w", "투명 픽셀 0 - 둥근 모서리 바깥도 어두운 색으로"),
              ("w", "맨 아래 큰 사각형(플레이어 인벤)에는 격자도 장식도 금지"),
              ("w", "좌우 프레임은 한쪽 16px 이내")],
    "inventory": [("w", "캔버스 크기 그대로. 1px도 다르면 안 된다"),
                  ("w", "투명 픽셀 0 - 둥근 모서리 바깥도 어두운 색으로"),
                  ("w", "칸을 옮기거나 크기를 바꾸지 말 것 - 좌표가 코드에 박혀 있다"),
                  ("w", "왼쪽 테두리는 28px 이내 (방어구 칸이 바로 붙어 있다)"),
                  ("w", "창 밖(오른쪽·아래 어두운 부분)으로 그림이 넘어가면 안 된다")],
    "dialogue": [("w", "가로세로 비율 5.5:1 (440x80) 을 지킬 것"),
                 ("w", "투명 배경 허용 - 판 바깥은 비워도 된다(화면에 얹히는 그림이라)"),
                 ("w", "초상화·이름표·대사 자리에 그림을 채우지 말 것"),
                 ("w", "110px 경계에 큰 장식을 걸치지 말 것")],
}

SHEETS = {
    "smithy": ("대장간 허브", [
        ("p", "NPC에게 말 걸면 처음 나오는 화면. 작업대가 넷뿐이라 2x2로 두고"),
        ("p", "사이와 바깥에 한 칸씩 여백을 뒀다."),
        ("s", ""),
        ("hi", "★비어 있는 곳이 이 화면의 주인공이다."),
        ("p", "타일 사이 골, 좌우 여백, 위 한 줄 - 전부 이어진 대장간 내부로."),
        ("p", "모루 · 풀무 · 화덕 · 물통 · 걸린 집게와 망치 · 세워둔 쇠막대."),
        ("p", "타일 4개는 그 공간에 박아 넣은 놋쇠 명판처럼. 안은 비운다."),
        ("p", "오른쪽 위 홈 하나는 닫기 버튼 자리."),
    ]),
    "crafting": ("조합대", [
        ("p", "왼쪽에 레시피 목록(5x5), 오른쪽에 3x3 제작 그리드."),
        ("p", "왼쪽은 벽에 붙인 도면/장부, 오른쪽 3x3은 작업판으로 갈라 보이게."),
        ("hi", "3x3은 한 덩어리로 묶여 보여야 한다 - 조합대라는 게 읽혀야 한다."),
        ("p", "아래쪽 홈 하나가 제작 버튼 - 눌러야 할 것처럼 강조."),
    ]),
    "disassemble": ("부품 분해", [
        ("p", "넣고 → 갈기. 가운데 3x7 초록칸에 부술 부품·재료를 넣고"),
        ("p", "그 아래 가운데 홈 하나가 「갈기」 버튼이다. 그게 전부다."),
        ("s", ""),
        ("hi", "★칸이 적은 대신 배경이 넓다."),
        ("p", "숫돌 · 정 · 모루 위 부서진 쇳조각 · 쇳가루를 넉넉하게."),
        ("p", "21칸은 하나의 분쇄 상판으로 묶여 보이게(칸마다 액자 금지)."),
        ("p", "갈기 홈은 이 화면의 유일한 동작 - 가장 눈에 띄게."),
    ]),
    "forge": ("재료 제작소", [
        ("p", "분해의 반대. 위 한 줄에 홈 둘(분해로 가기 · 닫기), 아래 36칸 목록."),
        ("p", "거푸집 · 도가니 · 정련. 분해가 부수는 방이면 여기는 빚는 방이다."),
        ("p", "분해창과 짝이라 색은 이어지되 도구가 반대면 좋다."),
    ]),
    "iceshop": ("아이스박스 상점", [
        ("p", "아이스박스를 한 단계씩 올리는 화면. 3행 중 가운데 줄 9칸만 진열이고"),
        ("p", "위아래 한 줄씩은 여백이다. 홈은 그 9개 + 왼쪽 위 뒤로가기 하나."),
        ("p", "잔액·진주 보유량은 제목 줄에 뜨고 닫기는 Esc라 버튼이 없다."),
        ("p", "왼쪽 위 홈 하나는 보관함으로 돌아가는 뒤로가기 자리다."),
        ("s", ""),
        ("hi", "★가운데 9칸이 주인공, 위아래 여백이 조연이다."),
        ("p", "9칸은 왼쪽에서 오른쪽으로 좋아지는 계단처럼 - 왼쪽은 소박한 나무 선반,"),
        ("p", "오른쪽으로 갈수록 얼음이 두껍고 서리가 짙어지게."),
        ("p", "마지막 칸만은 전설급이라 티가 나도 좋다(빛나는 테두리 정도)."),
        ("p", "위아래 여백은 진열대를 감싸는 창고로 - 위는 고드름 매달린 천장,"),
        ("p", "아래는 성에 낀 바닥이나 얼음 벽돌 단. 진열이 공중에 뜨지 않게."),
    ]),
    "skillhub": ("스킬 허브 - 재작업", [
        ("p", "받은 그림(오른쪽 아래 참고)은 재질·색·구도 다 좋다. 그대로 간다."),
        ("p", "고칠 건 하나뿐이다 - 액자 5개의 간격이 어긋나 있다."),
        ("s", ""),
        ("hi", "★액자 다섯의 간격이 76px인데 72px 이어야 한다."),
        ("p", "게임은 아이템을 정확히 72px 간격에 그린다. 그래서 지금 그림이면"),
        ("p", "양 끝 아이콘이 액자 밖으로 9px씩 삐져나온다."),
        ("p", "액자 중심 x = 208 · 280 · 352 · 424 · 496 (가운데는 이미 맞다)."),
        ("p", "세로 중심 y = 176 도 이미 맞다 - 건드리지 말 것."),
        ("s", ""),
        ("hi", "★액자 안쪽 구멍은 한 변 68px 이상."),
        ("p", "지금 52px 이라 64px 짜리 아이콘이 테두리를 덮는다."),
        ("p", "테두리는 그만큼 바깥으로 - 액자 바깥 한 변 72px 까지 써도 된다."),
        ("s", ""),
        ("p", "왼쪽 판의 회색 홈이 정확한 자리다. 그 위에 액자를 얹으면 된다."),
    ]),
    "skilltree": ("특성 트리", [
        ("p", "스킬 하나를 눌렀을 때 나오는 상세. 0행에 버튼 4개(이전·정보·다음·초기화),"),
        ("p", "그 아래는 전부 특성 노드와 연결선이다 - 왼쪽 열에는 근원 노드가"),
        ("p", "하나 있고 거기서 세로 연결선이 아래로 뻗어 각 갈래로 이어진다."),
        ("s", ""),
        ("hi", "★점선 안(1~4행 전체, 왼쪽 열 포함)에는 홈을 파지 말 것."),
        ("p", "노드와 연결선이 칸 밖까지 넘쳐 그려지는 아이콘이라, 소켓 테두리가 있으면"),
        ("p", "선과 겹쳐 지저분해진다. 그 영역은 평평한 판이어야 한다."),
        ("p", "판은 조용하게 - 특성 아이콘이 주인공이고 배경은 벽지다."),
        ("p", "성좌도·설계도·족보 같은 느낌. 은은한 결이나 격자 정도까지만."),
        ("s", ""),
        ("hi", "★검은 여백을 만들지 말 것."),
        ("p", "지금 판의 가장 큰 불만이 그거다. 어둡게 하되 재질이 보이게."),
    ]),
    "inventory": ("생존 인벤토리 (E 키)", [
        ("p", "지금까지 그린 건 우리가 띄우는 창이었고, 이건 클라이언트가 그리는"),
        ("p", "바닐라 인벤토리다. 칸 위치가 코드에 박혀 있어 못 옮긴다 -"),
        ("p", "왼쪽 판의 회색 칸이 정확한 자리이고, 1px도 어긋나면 아이템이 튄다."),
        ("s", ""),
        ("hi", "★파란 사각형 안에는 플레이어 3D 모델이 그려진다."),
        ("p", "그 뒤에 깔 배경만 그린다 - 인물 사진을 넣을 액자라고 생각하면 된다."),
        ("p", "가운데를 밝게 두면 캐릭터가 산다. 무늬를 빼곡히 넣지 말 것."),
        ("s", ""),
        ("hi", "★빨간 사각형에는 바닐라 레시피책 버튼이 얹힌다."),
        ("p", "장식을 넣어도 가려지니 조용히 둘 것."),
        ("s", ""),
        ("hi", "★칸을 네 무리로 읽히게."),
        ("p", "왼쪽 세로 4칸(방어구) · 그 아래 하나(보조손) · 오른쪽 위 2x2+결과(조합)"),
        ("p", "· 아래 3x9 가방과 한 줄 띄운 단축바. 무리마다 테두리나 바닥이 다르면 좋다."),
        ("p", "단축바 한 줄은 특히 도드라지게 - 손에 쥔 것들이다."),
    ]),
    "dexmain": ("도감 메뉴", [
        ("p", "/도감 을 열면 처음 나오는 화면. 가운데 줄 일곱 칸이 분류 버튼이고"),
        ("p", "(물고기·낚싯대·부품·재료·지역·수집품·채집품) 나머지는 전부 그림이다."),
        ("s", ""),
        ("hi", "★도감 네 장은 한 권의 책이어야 한다."),
        ("p", "여기가 표지, 나머지 셋이 속장이다. 같은 종이·같은 가죽·같은 금박으로."),
        ("p", "이 화면만 표지답게 - 두꺼운 장정, 잠금쇠, 책등의 인장 같은 것."),
        ("p", "위 홈 하나는 제목 머리, 오른쪽 위 홈은 닫기다."),
    ]),
    "dextab": ("도감 속장 - 목록", [
        ("p", "낚싯대·부품·재료·지역·채집품 다섯 탭과 수집품 상세가 **이 판 하나**를 같이 쓴다."),
        ("p", "가운데 4x7(28칸)이 수집 칸, 왼쪽 위 뒤로가기, 오른쪽 위 닫기,"),
        ("p", "아래 가운데 셋이 이전·페이지·다음이다."),
        ("s", ""),
        ("hi", "★28칸이 책장에 붙인 표본판처럼 보이면 성공."),
        ("p", "칸마다 액자를 두르지 말고 한 덩어리 판에 홈을 판 느낌으로."),
        ("p", "빈 칸도 많이 보이는 화면이다(미발견) - 홈 안쪽은 조용하게."),
        ("p", "여러 화면이 쓰므로 특정 주제(물고기·광물 등) 그림은 넣지 말 것."),
    ]),
    "dexfish": ("도감 속장 - 물고기", [
        ("p", "지역별·전체·미분류 물고기 도감이 같이 쓴다. 가장 자주 열리는 도감이다."),
        ("p", "0행은 왼쪽 뒤로 · 오른쪽 닫기 · 사이 일곱 칸이 등급 필터(E~G),"),
        ("p", "9~44 서른여섯 칸이 어종, 맨 아랫줄이 통째로 내비게이션이다."),
        ("s", ""),
        ("hi", "★윗줄(등급 필터)과 아랫줄(내비)이 목록과 갈라져 보여야 한다."),
        ("p", "가운데 36칸은 하나의 진열장, 위아래 줄은 그 틀에 붙은 손잡이처럼."),
        ("p", "여기만 물빛을 조금 써도 된다 - 물고기 도감이니까. 다만 다른 속장과"),
        ("p", "같은 종이·같은 금박은 유지할 것."),
    ]),
    "dexisland": ("도감 속장 - 수집품 섬 목록", [
        ("p", "수집품 도감의 첫 화면. 섬을 고르면 상세(도감 속장 - 목록)로 넘어간다."),
        ("p", "가운데 2x7(14칸)이 섬 목록이고 위 줄에 뒤로·머리·닫기 셋뿐이다."),
        ("s", ""),
        ("hi", "★네 행짜리라 아래가 비어 보이기 쉽다."),
        ("p", "목록 아래 여백을 지도 펼친 책상처럼 채워 달라 - 해도, 자, 조개 몇 개."),
        ("p", "그래도 종이·금박은 다른 속장과 같은 것으로."),
    ]),
    "dialogue": ("NPC 대화창", [
        ("p", "NPC에게 말 걸면 화면 아래에 뜨는 창. 상자 GUI가 아니라 BetterHud 가"),
        ("p", "화면에 직접 얹는 그림이라, 그린 크기가 곧 화면 픽셀이다."),
        ("p", "최종 440x80. 왼쪽 판은 4배 확대본이니 여기 덧칠하면 우리가 줄인다."),
        ("s", ""),
        ("hi", "★파랑=초상화 · 노랑=이름표 · 초록=대사. 셋 다 코드가 얹는다."),
        ("p", "그 자리는 파 두고 비운다 - 액자와 홈만 그리고 내용은 넣지 말 것."),
        ("p", "이름표는 판 아래로 8px 삐져나온다. 거기 걸리는 장식은 잘려 보인다."),
        ("s", ""),
        ("hi", "★110px 마다 세로로 잘린다(빨간 점선)."),
        ("p", "네 조각으로 쪼개 글리프로 굽기 때문이다. 경계에 딱 걸치는 큰 장식"),
        ("p", "(못·경첩·문양)은 이음매가 티 난다 - 조각 안쪽에 두거나 가로로 연속되게."),
        ("s", ""),
        ("hi", "★대사가 주인공이다."),
        ("p", "오른쪽 큰 면은 읽는 자리다. 무늬를 넣더라도 글자를 방해하지 않게"),
        ("p", "아주 옅게. 왼쪽 초상화 쪽은 반대로 무겁게 - 액자·장식은 거기 몰아준다."),
    ]),
    "workbench": ("장비 작업대", [
        ("p", "/장비 로 여는 창. 낚싯대에 달 부품을 끼우고 빼는 자리다."),
        ("p", "세 줄뿐이라 좁다. 쓰는 칸은 위 가운데 하나와 가운데 줄 다섯뿐."),
        ("s", ""),
        ("hi", "★가운데 줄 다섯 칸이 이 창의 전부다."),
        ("p", "플레이어가 인벤토리에서 부품을 **끌어다 넣는** 칸이다."),
        ("p", "그래서 '진열장'이 아니라 '꽂는 자리'로 보여야 한다 -"),
        ("p", "비어 있을 때 넣고 싶어지는 홈. 얕은 요람, 가죽 끈 고리, 놋쇠 클립 같은."),
        ("p", "다섯은 왼쪽부터 릴·줄·바늘·미끼·찌 순서로 고정이다."),
        ("p", "칸 바깥 여백(위아래)에 그 부품을 암시하는 음각이나 낙인을 넣어도 좋다 -"),
        ("p", "★단 홈 안쪽에는 아무것도 그리지 말 것. 부품 아이콘이 그 위에 얹힌다."),
        ("p", "★넷째 칸(미끼)은 개수 숫자가 칸 오른쪽 아래에 겹쳐 찍힌다(예 99)."),
        ("p", "  거기에 밝은 장식이 오면 숫자를 못 읽는다 - 그 모서리는 어둡고 조용하게."),
        ("s", ""),
        ("hi", "★첫째 줄 가운데 하나는 장비 요약이다."),
        ("p", "지금 낚싯대와 합산 스탯을 보여주는 자리. 다섯 칸의 '주인'처럼 보이게 -"),
        ("p", "위에 걸린 낚싯대, 명패, 펼친 장비 대장 같은. 나머지 첫 줄은 그림 몫이다."),
        ("p", "닫기 버튼이 없는 창이니 오른쪽 끝도 비워 두지 말고 그림으로 채울 것."),
        ("s", ""),
        ("hi", "★셋째 줄은 통째로 그림이다."),
        ("p", "쓰는 칸이 하나도 없다. 작업대 아래 서랍·공구걸이·감아둔 낚싯줄 뭉치 등"),
        ("p", "가장 자유롭게 쓸 수 있는 줄이다. 여기서 '작업장'다움을 만들어 준다."),
        ("s", ""),
        ("p", "톤: 항해사의 장비함을 펼쳐 놓은 작업대. 기름 먹인 나무, 놋쇠, 가죽."),
        ("p", "인벤토리 판과 한 세트로 보이게 - 같은 창에서 아이템을 주고받는 화면이다."),
    ]),
    "questlist": ("퀘스트 목록", [
        ("p", "쉬프트+Q 로 여는 창. 지금 뭘 하고 있는지 확인하는 자리다 -"),
        ("p", "새 퀘스트를 받는 곳이 아니라, 받아 둔 것을 들여다보는 곳."),
        ("s", ""),
        ("hi", "★가운데 큰 홈(2번째 줄) 하나가 지금 하는 메인 퀘스트다."),
        ("p", "이 창의 주인공. 다른 칸보다 격이 높아 보여야 한다 -"),
        ("p", "밀랍 봉인을 뜯은 서찰, 액자에 끼운 지도, 못 박은 수배서 같은."),
        ("p", "진행 중이 없으면 빈 채로 남으니, 비어도 허전하지 않게 안쪽을 채워 둘 것."),
        ("s", ""),
        ("hi", "★셋째 줄은 오늘의 일일 퀘스트다."),
        ("p", "맨 왼쪽 하나가 '오늘의 일일' 이름표고, 그 오른쪽 일곱이 목록이다."),
        ("p", "일곱은 매일 바뀌는 쪽지들 - 이름표와 목록이 한 줄로 이어져 보이게."),
        ("p", "목록 칸은 조용하게(내용물이 매번 바뀐다). 이름표만 조금 다르게."),
        ("s", ""),
        ("hi", "★첫째 줄은 머리띠다."),
        ("p", "가운데 하나가 제목 명판, 오른쪽 끝 하나가 닫기. 나머지 일곱은 그림 몫."),
        ("p", "항해 일지를 펼친 가죽 표지, 놋쇠 모서리쇠, 끼워 둔 리본 같은 것."),
        ("s", ""),
        ("p", "톤: 바르칸 열도의 여행자 수첩. 낡은 가죽과 양피지, 놋쇠 장식."),
        ("p", "도감(책)보다 사적이고 손때 묻은 느낌 - 남에게 보여주는 장부가 아니라 내 수첩."),
    ]),
    "questnpc": ("NPC 퀘스트 수락창", [
        ("p", "NPC가 퀘스트를 건네는 창. 두 줄뿐이라 아주 좁다."),
        ("p", "퀘스트를 고르는 창이 아니다 - 지금 받을 수 있는 **한 장**만 놓인다."),
        ("s", ""),
        ("hi", "★아래 줄 가운데 홈 하나가 전부다."),
        ("p", "그 한 자리가 의뢰서다. 크고 분명하게, 손이 저절로 가게."),
        ("p", "게시판에 압정으로 꽂은 양피지, 봉랍 찍은 서찰, 못 박은 의뢰서 같은."),
        ("p", "선행 퀘스트가 안 끝났으면 같은 자리에 잠금 표시가 뜬다 -"),
        ("p", "그러니 '받는 자리'이자 '못 받는 자리'로도 읽혀야 한다. 특정 색에 기대지 말 것."),
        ("s", ""),
        ("hi", "★첫째 줄은 머리띠, 나머지는 전부 그림이다."),
        ("p", "가운데 하나가 제목(누구의 퀘스트인지), 오른쪽 끝이 닫기."),
        ("p", "아래 줄도 가운데 하나 빼면 전부 배경이다 - 좁은 만큼 여백이 귀하다."),
        ("p", "양옆을 게시판 기둥이나 걸린 등불로 눌러 주면 가운데가 산다."),
        ("s", ""),
        ("p", "톤: 마을 게시판 앞. 나무판과 못, 바랜 양피지, 붉은 봉랍."),
        ("p", "퀘스트 목록(수첩)과 한 세트로 보이되, 이쪽이 더 바깥이고 공적인 자리다."),
    ]),
    "npcdialog": ("NPC 대화 창", [
        ("p", "NPC 대사가 끝나면 열리는 창. 화면 아래 대화 판(BetterHud)과 짝이다."),
        ("p", "위 두 줄(파랑 18칸)이 방금 나눈 대화 기록, 셋째 줄은 구분 줄과"),
        ("p", "가운데 하나(진행중 퀘스트), 넷째 줄 가운데 다섯이 선택지다."),
        ("s", ""),
        ("hi", "★대화 기록 18칸은 '읽는 줄'이지 진열장이 아니다."),
        ("p", "칸마다 액자를 두르면 글이 갇힌다. 줄 없는 종이에 가깝게 -"),
        ("p", "얕은 홈이나 옅은 괘선 정도까지만. 안쪽은 조용하게."),
        ("s", ""),
        ("hi", "★넷째 줄 다섯 칸이 이 창의 유일한 동작이다."),
        ("p", "누를 것처럼 보이게 - 밀랍 봉인, 놋쇠 명판, 눌러 붙인 쪽지 같은."),
        ("p", "다섯이 다 차는 일은 없다(최대 넷) - 가운데 하나만 쓰는 경우가 제일 흔하다."),
        ("p", "그러니 다섯이 한 덩어리로 묶여 보이면 안 된다. 각각 따로 서게."),
        ("s", ""),
        ("hi", "★셋째 줄은 갈라주는 줄이다."),
        ("p", "위(읽는 곳)와 아래(고르는 곳)를 눈으로 끊어 준다. 가운데 홈 하나에는"),
        ("p", "진행중 퀘스트가 올라간다 - 거기만 살짝 강조."),
    ]),
    "enhance": ("낚싯대 강화", [("p", "이미 납품 완료 - 만듦새·색의 기준이 되는 화면이다.")]),
    "guildupgrade": ("길드 업그레이드", [
        ("p", "/길드 → 업그레이드 로 여는 창. 길드가 돈을 써서 커지는 자리다."),
        ("p", "지금은 공용판을 쓰고 있는데, 칸 자리가 고정된 화면이라 전용판으로 뽑는다."),
        ("s", ""),
        ("hi", "★이 창은 좌우 두 덩어리다. 3x3 격자 두 벌."),
        ("p", "왼쪽 아홉 칸 = 길드 버프 · 오른쪽 아홉 칸 = 섬 업그레이드."),
        ("p", "위아래가 아니라 **좌우**로 갈린다. 같은 크기 같은 모양 두 벌이"),
        ("p", "가운데를 사이에 두고 마주 본다 - 좌우 대칭이 이 화면의 뼈대다."),
        ("s", ""),
        ("p", "왼쪽은 길드원 **몸에** 붙는 것 - 낚시가 잘 되고 발이 빨라진다."),
        ("p", "오른쪽은 길드섬이라는 **땅에** 붙는 것 - 넓어지고 더 놓을 수 있다."),
        ("p", "몸에 붙는 것과 땅에 붙는 것. 왼쪽은 걸리는 것(유리병·부적·깃발),"),
        ("p", "오른쪽은 쌓이는 것(돌·흙·기둥) 정도로 재질을 갈라 주면 한눈에 읽힌다."),
        ("p", "다만 **격은 맞출 것** - 한쪽만 화려하면 그쪽이 더 좋은 줄 안다."),
        ("s", ""),
        ("hi", "★가운데 세로줄 한 칸이 통째로 비어 있다."),
        ("p", "거기가 두 격자를 갈라 주는 자리다. 가로로 긋는 띠는 없앴다 -"),
        ("p", "이 빈 세로줄이 그 노릇을 한다. 위에서 아래로 흐르는 것이 어울린다:"),
        ("p", "기둥 · 늘어뜨린 현수막 · 사슬 · 벽의 이음매."),
        ("w", "이 세로줄에 홈을 파지 말 것 - 아무것도 안 올라간다"),
        ("s", ""),
        ("hi", "★맨 왼쪽·맨 오른쪽 세로줄도 통째로 프레임이다."),
        ("p", "결국 세로줄 셋(맨 왼쪽 · 가운데 · 맨 오른쪽)이 그림 몫이고,"),
        ("p", "그 사이 두 자리에 3x3 격자가 하나씩 앉는 구조다."),
        ("p", "홈은 격자 안에만 있다 - 왼쪽 위 뒤로가기 하나만 예외."),
        ("s", ""),
        ("hi", "★첫 줄과 마지막 줄도 그림이다."),
        ("p", "격자는 가운데 세 줄에만 앉는다. 위아래로 한 줄씩 여백이 있으니"),
        ("p", "천장과 바닥 - 회관의 위쪽 장식과 아래쪽 받침을 만들어 줄 자리다."),
        ("s", ""),
        ("p", "머리 둘은 각 격자 바로 위에 하나씩 붙는다(왼쪽 · 오른쪽)."),
        ("p", "격자의 이름표라 살짝 도드라져도 좋다. 둘은 같은 격으로."),
        ("p", "칸 위에 레벨이 글자로 찍히니 홈 안쪽은 조용하게."),
        ("s", ""),
        ("hi", "★아홉 칸을 다 채우지 않는다 - 그래도 아홉 칸 다 팔 것."),
        ("p", "지금 들어갈 것은 왼쪽 여섯 · 오른쪽 일곱이라 몇 칸이 빈 채로 열린다."),
        ("p", "늘릴 자리로 일부러 남긴 것이니 홈은 아홉 개 그대로 파 두고,"),
        ("p", "빈 칸이 '고장난 자리'로 보이지 않게 홈 자체를 조용하게 그릴 것."),
        ("s", ""),
        ("w", "닫기 버튼이 없는 창이다(Esc) - 오른쪽 아래를 비워 두지 말 것"),
    ]),
}

THEME_SMITHY = ("컨셉 - 대장간 한 채를 네 방으로",
         "강화창이 용광로 앞(뜨겁고 위험)이었다면 나머지는 그 대장간의 다른 구석이다.",
         "같은 재질(그을린 돌 · 달군 쇠 · 주황 불빛)을 쓰되 방마다 도구가 다르다.",
         "허브만 넓은 전경, 나머지 셋은 작업대 앞 클로즈업.")
THEME_ICE = ("컨셉 - 얼음 창고의 진열대",
             "아이스박스 보관함(src/icebox/bg_source.png)과 같은 방이다 - 그 화면의",
             "성에·고드름·푸른 얼음을 그대로 쓰되, 여기는 사러 온 손님이 보는 진열대다.",
             "9칸이 나무에서 전설까지 한 줄로 오르는 계단처럼 읽히면 성공.")
THEME_SKILL = ("컨셉 - 성장의 방",
               "허브는 다섯 갈래를 고르는 무대, 트리는 그중 하나를 파고드는 벽면이다.",
               "두 장이 같은 재질·같은 색으로 이어져야 한 시스템으로 읽힌다.",
               "밝고 정적인 톤 - 대장간(뜨거움)·아이스박스(차가움)와 구분되게.")
THEME_INV = ("컨셉 - 항해사의 장비함",
             "바르칸은 섬과 배의 서버다. 인벤토리는 그 항해사가 늘 여는 궤짝이면 좋겠다.",
             "젖은 나무 갑판 · 놋쇠 경첩과 못 · 밧줄 · 유리 아래 비치는 청록 물빛.",
             "색은 이미 쓰는 것들과 맞춘다 - 어두운 목탄·놋쇠·청록(대장간/스킬창 계열).",
             "★어둡되 검은 여백은 금지. 재질이 보이는 어둠으로.")
THEME_GUILD = ("컨셉 - 길드 회관의 게시벽",
               "메뉴·내 정보와 같은 타일형 화면이다. 그 둘과 한 세트로 보여야 한다 -",
               "어두운 나무 · 놋쇠 · 청록. 다만 여긴 '우리 것'이라는 자부심이 붙는다:",
               "벽에 건 현수막, 못 박은 명패, 촛대. 개인 화면(내 정보)이 책상이라면",
               "여긴 여럿이 드나드는 회관 벽이다.",
               "★타일 여섯이 나란히 선다 - 배경이 시끄러우면 버튼이 안 읽힌다.")

THEME_DEX = ("컨셉 - 항해일지 한 권",
             "도감 네 장은 낱장이 아니라 한 권이다. 표지 한 장 + 속장 셋.",
             "누렇게 바랜 종이 · 가죽 장정 · 금박 테두리 · 눌러 말린 표본과 핀.",
             "인벤토리(항해사의 장비함)와 같은 궤짝에 들어 있는 책처럼 -",
             "나무·놋쇠 대신 종이·가죽이지만 색온도는 같게 맞춘다.",
             "★어둡되 검은 여백은 금지. 재질이 보이는 어둠으로.")
THEME_NPC = ("컨셉 - 뱃사람의 게시판",
             "인벤토리(궤짝)·도감(항해일지)과 한 세트다. 같은 나무·놋쇠·양피지.",
             "초상화 쪽은 벽에 박은 놋쇠 액자, 대사 쪽은 거기 압정으로 눌러 붙인 쪽지.",
             "말하는 사람이 먼저 보이고 그다음 글이 읽히면 성공.",
             "★화면 아래 가운데에 늘 떠 있는 판이다 - 시끄러우면 금방 질린다.")

THEME_QUEST = ("컨셉 - 여행자의 수첩과 마을 게시판",
               "도감(항해일지)·NPC 대화(게시판)와 한 세트다. 같은 나무·놋쇠·양피지.",
               "다만 이쪽은 손때가 더 묻어 있다 - 접힌 자국, 압정 자국, 번진 잉크.",
               "목록은 내가 들고 다니는 수첩이라 사적이고,",
               "수락창은 마을 게시판 앞이라 공적이다. 둘이 같은 세계로 보이되 거리가 있게.",
               "★칸이 적은 창이다. 장식이 주인공(가운데 홈)을 밀어내면 안 된다.")

THEME_GUILD_UP = ("컨셉 - 회관 안쪽, 나란히 걸린 두 벌의 문장",
                  "길드 허브(회관 게시벽)에서 한 칸 들어간 방이다. 같은 어두운 나무 ·",
                  "놋쇠 · 청록을 쓰되, 게시벽이 '보는 곳'이라면 여긴 '올리는 곳'이다.",
                  "왼쪽은 길드원의 몸에 거는 것, 오른쪽은 섬이라는 땅에 쌓는 것 -",
                  "같은 벽에 나란히 걸린 두 벌의 판이다. 좌우가 거울처럼 맞아야 산다.",
                  "가운데 세로줄이 그 둘을 갈라 준다(기둥 · 현수막 · 사슬).",
                  "돈을 써서 길드가 커지는 화면이라 - 값이 나가 보이면 좋다.",
                  "★칸이 스무 개다. 배경이 시끄러우면 버튼이 안 읽힌다.")

THEMES = {"iceshop": THEME_ICE,
          "guild": THEME_GUILD, "guildupgrade": THEME_GUILD_UP,
          "workbench": THEME_INV,
          "questlist": THEME_QUEST, "questnpc": THEME_QUEST, "skillhub": THEME_SKILL, "skilltree": THEME_SKILL,
          "inventory": THEME_INV,
          "dexmain": THEME_DEX, "dextab": THEME_DEX,
          "dexfish": THEME_DEX, "dexisland": THEME_DEX,
          "dialogue": THEME_NPC, "npcdialog": THEME_NPC}

# 재작업 시트용 - 오른쪽 설명판 아래에 「이 그림 기준」으로 축소해 붙인다.
# 받은 그림을 말로만 가리키면 못 찾는다(발주서/그림 분리 사고와 같은 이유).
REFERENCE = {"skillhub": ("src/skillhub/bg_source_rebuild.png",
                          "받은 그림 - 재질·색·구도는 이대로")}


def font(px, bold=True):
    try:
        return ImageFont.truetype(FONT_TTF, px)
    except Exception:
        return ImageFont.load_default()


LEGEND = {
    "타일": "큰 황토색 판 = 큰 버튼. 안은 비운다(아이콘은 우리가 올림)",
    "홈": "회색 = 홈. 우리가 아이콘을 올린다",
    "입력": "초록 = 넣는 칸. 회색과 확실히 구분되게",
    "목록": "파랑 = 목록. 얕은 홈, 안쪽은 조용하게",
    "장식": "빈 자리 = 장식. 마음껏",
}


def grid_for(name):
    """칸 좌표를 숫자로 박는다 — '72px 칸' 같은 말만으로는 격자가 안 맞아서(2026-08-12
    도감 두 판을 코드로 옮기다 접힘선·걸쇠가 깨졌다) 자로 잴 수 있는 두 점을 준다."""
    if name not in L.PAGES:
        return []
    _, roles, _ = L.PAGES[name]
    # 기준점은 **격자(목록/입력)** 에서 잡는다 — 상단 버튼은 격자와 따로 놓이는 일이 많다.
    used = sorted(k for k, (r, _) in roles.items() if r in ("목록", "입력")) \
        or sorted(k for k, (r, _) in roles.items() if r != "장식")
    if not used:
        return []
    cx = lambda c: (L.GRID_X + L.CELL * c) * L.SCALE + 36
    cy = lambda r: (L.GRID_Y + L.CELL * r) * L.SCALE + 36
    fr, fc = divmod(used[0], L.COLS)
    lr, lc = divmod(used[-1], L.COLS)
    return [("w", "★칸 중심 간격(피치)은 가로·세로 모두 정확히 72px"),
            ("w", f"  첫 칸 구멍 한가운데 = ({cx(fc)}, {cy(fr)})"),
            ("w", f"  끝 칸 구멍 한가운데 = ({cx(lc)}, {cy(lr)})"),
            ("w", "  이 두 점만 맞으면 사이 칸은 저절로 맞는다")]


def legend_for(name):
    if name == "dialogue":
        return [("k", "파랑 = 초상화 · 노랑 = 이름표 · 초록 = 대사"),
                ("k", "빨간 점선 = 110px 조각 경계"),
                ("k", "그 밖 = 장식. 마음껏")]
    if name not in L.PAGES:          # 상자 창이 아닌 화면(바닐라 인벤토리 등)
        return [("k", "회색 = 아이템이 들어가는 칸. 안은 비운다"),
                ("k", "파랑 = 플레이어 3D 모델 자리 · 빨강 = 바닐라 버튼 자리"),
                ("k", "그 밖 = 장식. 마음껏")]
    rows, roles, default = L.PAGES[name]
    used = {r for r, _ in roles.values()} | {default}
    if L.TILES.get(name): used.add("타일")
    return [("k", LEGEND[r]) for r in ("타일", "홈", "입력", "목록", "장식") if r in used]


def build(name):
    title, body = SHEETS[name]
    plate_path = os.path.join(HERE, "src", name, "_template.png")
    plate = Image.open(plate_path).convert("RGBA")
    lines = 0
    sections = body + [("s", "")] + COMMON
    hard = HARD.get(name, HARD["chest"])
    sections = [x for k, t in sections
                for x in (legend_for(name) if k == "legend" else hard if k == "hard"
                          else grid_for(name) if k == "grid" else [(k, t)])]
    for kind, text in sections:
        lines += {"s": 0.4, "t": 1.5}.get(kind, 0)
        if kind not in ("s", "t"):
            lines += max(1, len(textwrap.wrap(text, 34)))
    theme = THEMES.get(name, THEME_SMITHY)
    wrapped = [w for line in theme[1:] for w in textwrap.wrap(line, 40)]
    ref = REFERENCE.get(name)
    ref_h = 0
    if ref:
        ri = Image.open(os.path.join(HERE, ref[0])).convert("RGBA")
        rw = PANEL_W - 56
        ri = ri.resize((rw, round(ri.height * rw / ri.width)), Image.LANCZOS)
        ref_h = ri.height + 60
    need = 170 + int(lines * 30) + 40 + 34 + 26 * len(wrapped) + 30 + ref_h
    W, H = plate.width + PANEL_W, max(plate.height, need)
    im = Image.new("RGBA", (W, H), BG + (255,))
    im.alpha_composite(plate, (0, (H - plate.height) // 2))
    d = ImageDraw.Draw(im)
    d.line([plate.width, 0, plate.width, H], fill=(70, 70, 78, 255), width=3)

    x, y = plate.width + 28, 26
    d.text((x, y), f"발주 - {title}", font=font(38), fill=HI); y += 54
    d.text((x, y), f"{plate.width} x {plate.height} · 왼쪽 판 위에 덧칠", font=font(20), fill=DIM); y += 20
    d.text((x, y + 8), "이 오른쪽 설명은 잘라내고 왼쪽만 쓰면 된다", font=font(20), fill=DIM); y += 46

    for kind, text in sections:
        if kind == "s":
            y += 12; continue
        if kind == "t":
            y += 8
            d.text((x, y), text, font=font(26), fill=HI); y += 38; continue
        col = {"p": INK, "k": DIM, "w": WARN, "hi": HI}[kind]
        mark = {"k": "· ", "w": "! ", "hi": "", "p": ""}[kind]
        for i, line in enumerate(textwrap.wrap(mark + text, 34) or [""]):
            d.text((x + (0 if i == 0 else 20), y), line, font=font(22), fill=col)
            y += 30

    y += 28
    d.text((x, y), theme[0], font=font(24), fill=HI); y += 34
    for line in wrapped:
        d.text((x, y), line, font=font(19), fill=DIM); y += 26

    if ref:
        y += 30
        d.text((x, y), ref[1], font=font(20), fill=DIM); y += 28
        im.alpha_composite(ri, (x, y))

    out = os.path.join(HERE, "src", name, "_order.png")
    im.convert("RGB").save(out)
    print(f"  {name:12} {W}x{H} → {out}")


if __name__ == "__main__":
    for n in (sys.argv[1:] or ["smithy", "crafting", "disassemble", "forge"]):
        build(n)
