...
PLATES = {
    ...
    # 수리(/수리) — 장착 부품·통발 입력칸 + 수리 실행 버튼 전용 3행 판.
    "repair": (3, "repair_", 0xE870),
    # 유물 감정 — 사막 감정대. 중앙 입력칸 + 우하단 닫기.
    "artifact": (3, "artifact_", 0xE880),
    # 말 대여소 — 마구간 진열대. 잔액·말 4종·활성 대여 소환.
    "horse": (3, "horse_", 0xE890),
    # 배 대여소 — 항구 진열대. 보트 4종·활성 대여 소환.
    "boat": (3, "boat_", 0xE8D0),
    # 배낭 상점 3행 — 가죽·황동 티어 진열대.
    "backpack_shop": (3, "bpshop_", 0xE8E0),
    # 커스텀 인첸트 3행 — 룬 작업대와 입력·결과 슬롯.
    "enchant": (3, "enchant_", 0xE8F0),
    # 직거래 5행 — 양측 3x3 교환대와 확인 버튼.
    "trade": (5, "trade_", 0xE900),
    # 초음파 탐지기 6행 — 수중 관측실 물고기 목록.
    "fish_detector": (6, "fishdet_", 0xE910),
    # 제출소 6행 — 섬·길드 기여 물품 보드.
    "submit": (6, "submit_", 0xE920),
    # 쓰레기통 6행 — 45칸 투입부와 비우기·안내·닫기 버튼.
    "trash": (6, "trash_", 0xE930),
    # 요리 배우기 확인 3행 — 레시피 미리보기·구매·취소.
    "cooking_confirm": (3, "cookconfirm_", 0xE940),
    # 배 블록 편집기 3행 — 블록 정보·애니메이션 그룹·스케일 프리셋.
    ...
    # 길드 목록·설정·명부·신청·역할·요리 등 화면 공용 관리판.
    # 버튼/목록은 화면별 코드가 올리므로 판에는 전용 액자와 빈 콘텐츠 패널만 굽는다.
    "guild_manage": (6, "guildmg_", 0xE8C0),
}
