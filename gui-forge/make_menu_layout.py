...
## 배치 (54칸, 6행)
    0행        제목 명판 (버튼 없음)  ← ★타일을 0행부터 놓으면 제목 자리를 먹는다
    1~2행      큰 타일 3개  (3칸 x 2칸)
    3~4행      큰 타일 3개
    5행        작은 아이콘 8개 + 닫기 (한 줄 꽉)
...
TILE_COLS = [0, 3, 6]
# ★섬·길드를 '내 공간'으로 묶지 않는다 — 둘 다 자체 서브시스템(길드: 목록·랭킹·설정·
#   업그레이드·기여랭킹·요리 / 섬: 워프·업그레이드·인원·권한)이라, 위에 선택 화면을
#   얹으면 메뉴→내 공간→길드→길드설정 으로 4단계가 된다.
#   대신 얕은 '상점'(특수 재화 3종)을 아이콘으로 내렸다.
TILES = [
    ("내 정보", 0, 1), ("레벨·특성", 3, 1), ("장비", 6, 1),
    ("퀘스트", 0, 3), ("내 섬", 3, 3), ("길드", 6, 3),
]
# 4행을 9칸 꽉 채운다 — 한 칸이라도 비면 다시 골이 생긴다.
# 도감은 '내 정보' 허브로 옮겼다(내가 발견한 기록이라 거기 어울리고, 내 정보 타일이
# 5개면 3열 배치에서 한 칸이 비어 골이 생긴다). 빈 자리는 팁으로 메워 9칸을 유지.
ICONS = [("상점", 45), ("아이스박스", 46), ("거점", 47), ("잠수", 48), ("이모트", 49),
         ("배경음악", 50), ("채팅", 51), ("팁", 52)]
SLOT_CLOSE = 53
