...
- **로그인은 유저가 한다.** 인앱 브라우저에 세션이 없으면 "패널 열고(Ctrl+]) 로그인해 주세요"
  하고 멈춘다. 비밀번호는 브라우저 타이핑이든 스크립트든 대신 입력하지 않는다. 세션 쿠키는
  남으니 로그인은 몇 주에 한 번이고, 그 뒤 클릭은 전부 이 스킬이 한다.
- **끌어올리기는 유저가 말할 때 한 번씩 누른다.** cron·loop 로 걸지 않는다 — 쿨타임 걸린
  참여 지표를 반복 클릭하면 리스팅이 제재된다. 쿨타임 종료 «알림»만 띄우는 건 괜찮다.
- 소개글·서버 정보는 명시 요청 시에만 수정한다.

## 1. 서버 끌어올리기

★**버튼이 페이지에 안 보인다.** `관리` 모달 안에 있다. `find` 로 ref 는 잡히는데 화면에는
없어서 바로 클릭하면 실패한다. 반드시 모달을 먼저 열 것.

```
1) mcp__Claude_Browser__navigate  →  https://minelist.kr/servers/17441-barkan.kr
2) get_page_text 로 로그인 상태 확인 (우상단에 wsi1212 가 보이면 로그인됨)
   · 패널이 숨겨져 있으면 read_page 가 "(empty page) Viewport: 0x0" 을 낸다.
     텍스트는 get_page_text 로 읽히지만, 유저가 로그인해야 하면 Ctrl+] 로 패널을 열라고 한다.
3) `관리` 버튼 클릭 → "서버 관리" 모달
4) 모달에서 `서버 끌어올리기` 행을 읽는다:
   · "17:06부터 다시 가능"  → 쿨타임. 누르지 말고 남은 시간을 알려준다.
   · 쿨타임 문구가 없으면    → 클릭한다.
5) 클릭 후 screenshot 으로 결과 확인(성공 토스트 / 순위 변화).
```
