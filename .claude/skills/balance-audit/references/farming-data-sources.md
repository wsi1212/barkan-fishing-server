...
재배 스킬(`SkillManager.FARMING`). 레벨 엔진은 `SkillManager.addExp`(채굴·재배·요리·수집 4스킬 공통):
시작 need=100, 매 레벨 `need=floor(need×needMul)`, 근원특성(`농사근원` perRank2.0×5=최대+10%) 곱적용.

**★ 2026-08-01 너프 — 재배만 곡선 분리 (사유: ×1.06 곡선 + 플롯 병렬재배로 밀 32플롯 만렙 11h 과속).**
`SkillManager`에 `isFarming` 분기 신설 — **재배만** 별도 곡선·만렙 적용, 채굴/요리/수집은 ×1.06·만렙50 유지:
- **재배**: 만렙 **100**, `needMul`=×1.13(1~69레벨)→×1.10(70~레벨). 만렙(100) 누적 75,696,815 XP.
  - 1→50 누적 292,927(밀 32플롯 ~203h) · 50→60 702,775(~488h) · 60→70 2,385,541(~1,657h)
  · 70→80 7,006,508(~4,866h) · 80→90 18,173,014 · 90→100 47,136,050.
- **채굴/요리/수집**: 만렙 **50** 유지, `needMul`=×1.06(원복).
- ★근원 특성 +10%는 만렙 시간을 0.91배 단축 (재배 풀투자 시 1→50 185h).
...
2. **바닐라 작물** (`SkillManager.onHarvest` L171, WHEAT/CARROTS/POTATOES/BEETROOTS 완전성장 파괴) =
