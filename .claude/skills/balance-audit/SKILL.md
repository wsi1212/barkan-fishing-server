...
| `cast_cost.py` | **요구 캐스트**(장비 1개 = 몇 캐스트) 의 단일 권위 — κ 등위회귀 사다리 + 카테고리 총량 보존 + 동레벨 이상치 hold. 적용은 `fish-tools/patch_cast_cost.py` | ✅ 권위 |
...
## 요구 캐스트 (2026-08-27 신설)

**재료 요구 수량을 손으로 고치지 말 것.** `scripts/cast_cost.py` 가 목표를 내고
`fish-tools/patch_cast_cost.py` 가 정수 수량을 맞춘다. 근거는 감사노트
`audits/2026-08-27-cast-cost-spawn-village.md`.

    요구캐스트 = LP 재료게이트(h) × 249.1 캐스트/h
    목표       = κ(카테고리, 등급) × 순성능(원/h) / 1000

  · κ 는 «성능 1,000원/h 당 캐스트» = 캐스트로 잰 회수시간. **손으로 정하지 않는다** —
    현재 (카테고리, 등급) 중앙값의 **단조감소 등위회귀**로 낸다(최소 변경 + 단조성 강제).
  · 마지막에 **카테고리 총량 보존 배수**를 곱한다. 그래서 재분배만 일어나고 총량은 안 변한다.
  · **E 급은 사다리 밖**(튜토). 성능 모델이 잡음이라 κ 가 D 의 32배까지 튄다.

### ★동레벨 성능 이상치는 «hold» — 재료로 덮지 않는다
같은 레벨에서 성능이 25% 넘게 벌어지면 그건 **스탯 사다리 결함**이다. 두 가지 처방을
모두 시도해 봤고 둘 다 틀렸다:
...
### 정수 수량을 맞출 때
  ...
  · **모양 벌점 0.05** 를 반드시 걸 것. 없으면 「병목 재료 하나만 8→1」로 수렴해 캐스트는
    맞고 레시피는 망가진다.
  · 격자가 성글면(정제된갈고리 1개 = 146 캐스트) **중간재를 원재료로 되돌린다** —
    단, **E·D 등급만**, 그리고 **단단한자루는 절대 되돌리지 않는다**(자루 없는 작살 +
    물고기비늘 33개). 정체성이 정확도보다 앞선다.

회귀 방어는 `selftest.py [9]`.


## 작살 스탯 — 죽은 스탯 두 개 (2026-08-27)

`harpoon_value.Model` 유한차분(Lv10/24/45 동일):
`공격력 +22,433원/h/점` ≫ `도망감소 178` > `호흡시간 86` > `수중호흡 36` > `수영속도 14`
> **`공격속도 0` · `돌진쿨감 0`**.

  · **공격속도는 구조적으로 못 살린다** — 코드 쿨타임 `JAB_GAP_TICKS = 5`(0.25초)인데 실측
    인간 조준 간격이 **1.295초**다. 상한을 낮춰도 사람이 그보다 빨리 못 찌른다. 기준선을
    1.3초 위로 올려야 비로소 묶이는데 그건 전 작살 너프다. → **빌드에서 제거**(도망감소로 환산).
  · **돌진쿨감은 문턱 스탯** — 돌진 2회 조건 `10초/(1+쿨감/100) ≤ 교전창`. 단독이면 **60+**,
    도망감소 20 과 짝지으면 **40+** 부터 작동한다. 45 미만은 넣지 말 것.

### 등급 = 사냥터, 라인 = 그 안에서의 특화
    공격력 1 → B 등급까지 (A 는 6타 7.8초 > 창 6.0초로 불가)
    공격력 2 → **전 등급 해금** (S 4타 5.2초 < 6.0초)
그래서 D(공1)와 C(공2) 사이에 **4.5배 절벽**이 있고 그건 의도다. ★공격력을 라인 손잡이로
쓰지 말 것 — 1점이 게이트를 넘겨 같은 등급 안에서 4.5배를 만든다(쇠날 작살이 그랬다).
★도망감소도 교전창을 늘려 등급을 해금한다 — D 라인의 도망감소는 **8 이하**로 묶는다.
