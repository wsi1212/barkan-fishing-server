...
### 작업 원본은 **레포**(`ops/blockship-data/`)다 — 라이브가 아니다
이번 세션에 병렬 세션이 라이브 `recipes.json` 을 **두 번 통째로 덮어써** 패치 98건이 날아갔다
(08:55, 10:01). 레포에 대고 작업하고 마지막에 «수술식»으로 라이브에 내려보낼 것:
필요한 키만 옮기고 남의 변경(통발 지역 문구 등)은 건드리지 않는다.
    patch_*.py "$PWD/ops/blockship-data" --apply     ← 원본
    그 뒤 레포 → 라이브 ingredients/스탯만 머지
