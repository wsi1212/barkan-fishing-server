...
### 착수 진단 실측 — 사막 · 상단 · 왕도 (2026-08-27, `village_scan.py`)

    작살    B·A 에 **관통 · 채집 · 깡스탯 라인이 전무**
    부품 4슬롯  전 등급에 **숙련 · 성장 라인이 전무** (상인·행운·채집·크리로만 채워져 있다)
    낚싯대  B 에 숙련 · 크리 없음 (A 는 6라인 다 있음)
    κ 역전  낚싯대 B 34.6 → A 28.9 (상위 등급이 더 싸다 — 사다리 결함)
    죽은 스탯 돌진쿨감 문턱 미달 3종(모래바람 36 · 열풍 24 · 유목상단 16)

이게 «채워야 할 목록»이다. 라인을 새로 만들 때는 `rod_lines.DESIGN` / `spear_lines.ASSIGN` /
`part_lines` 에 그 마을 항목을 추가하고 절차 2 → 4 를 돌린다.
