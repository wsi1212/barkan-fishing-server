...
# ★2026-08-05 3차 갱신 — 앞선 두 차례(부품 5종 정체성 재배정 직후, 이후 난이도12→37 1차 교정)
#   전부 **손으로 어림한 값**이었고, 그 자리에서 실제 parts.json+enhance.json을 스캔하지 않아
#   두 군데가 틀렸다: ①S급 로드(바르칸·천공) 둘 다 "숙련형" 빌드를 안 써서 PRIMARY 이론상한
#   (S=8)에 닿는 로드가 실제로 하나도 없다(전부 FLOOR값 5). ②강화표 80개 로드 전체를 스캔한
#   실측 난이도 누적 최댓값은 1(4 아님, "참나무 낚싯대" Lv10). 이번엔 라이브 데이터를 코드로
#   직접 스캔해 전 스탯을 한 번에 재확정했다(카탈로그 best-of-type 합 + 강화표 실측 최댓값).
#   난이도 37→31(로드5+강화1+부품5×5=25), 트리플찬스 20→9(성장형이 A급엔 없고 C급뿐이라
#   부품 기여가 개당 1로 묶임 — 20은 "부품도 A급 성장형 있다"는 잘못된 가정이었다).
MAX_MAGNITUDE = {
    "판매보너스 (1%)": 108, "더블찬스 (1%)": 85, "트리플찬스 (1%)": 9,
    "등급업 (1%)": 65, "크기 (1%)": 69, "행운 (1점)": 103, "도주감소 (1%)": 30,
    "크리확률 (1%)": 95, "크리배율 (1점)": 8, "경험치 (1%)": 255, "난이도 (1점)": 31,
}
