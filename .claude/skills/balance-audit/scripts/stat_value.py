...
# ★★2026-08-26 전면 재산출 — 구 표(D 0.35 / C 0.80 / B 1.35 / A 2.01 / S 3.75)는 세 군데가
#   틀렸다. 경위·근거: audits/2026-08-26-material-chance-revaluation.md
#   ① 단위 오류 — "장비 1티어를 갖추는 시간"이라면서 실제로는 **아이템 1점** 게이트의 평균을
#      넣었다. 실제로 갖춰야 하는 건 낚싯대+릴+줄+바늘+찌 5~6점이고 재료 수량이 **합산**된다.
#   ② 확률 오기 — cross-economy-values.md §5가 별빛진주를 8%로 적었으나 materials.json 전 지역
#      실측은 **2%**(진주 8%와 혼동). A급 채집라인·A 미끼·상위 요리의 병목이 전부 별빛진주다.
#   ③ 지역 분산 무시 — 재료가 강/협곡/항구/정상/늪지대/오아시스에 흩어져 있어 한 자리에서
#      동시에 모을 수 없다. 아래 값은 «지역 안에서는 동시, 지역 간에는 순차» 모델(현실적 상한).
...
#   경험치   = 레벨 축 전용인데 1.00
#   판매보너스 = 수입 축 전용인데 1.00 (앵커)
#   재료확률  = 장비 축 전용인데 0.50   ← 여기만 절반
# 같은 논리를 다른 스탯에 적용하면 전부 절반이 되어야 한다. 축이 몇 개인지는 이미 아래
# sink_share(«그 국면에서 재료가 실제로 관문인 비율»)가 재고 있으므로 여기서 또 깎으면 중복이다.
GEAR_AXIS_SHARE = 1.0
...
# ★2026-08-26 종결 0.5 → 1.0. 구 0.5는 «A/S는 돈이 관문이니 절반만»이었는데, 위 게이트 재산출로
# A/S 풀세팅도 재료가 관문(12.73 vs 11.52 · 20.91 vs 16.98)임이 드러났고, 게다가 종결 재료 수요의
# 본체는 **돈으로 살 수 있는 경로가 아예 없는** 것들이다:
#   · 종결 작살 1자루 = 6~13.5h (천공의 작살 13.50h, 바르칸 작살 12.77h)
#   · 바르칸 대연회 1회 = 57.0h (별빛진주 150 = 2% 드롭 → 7,500 포획 = 34.1h 단독)
# 요컨대 종결에서 재료는 «절반짜리 관문»이 아니라 유일한 관문이다.
NON_GEAR_SINK_SHARE = {"초반": 1.0, "중반": 1.0, "종결": 1.0}
