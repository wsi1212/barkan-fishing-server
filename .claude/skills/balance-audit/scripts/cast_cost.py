...
GRADE_LIFT = float(os.environ.get("GRADE_LIFT", "1.00"))
#  ★환경변수로 덮을 수 있게 했다(2026-09-01) — 일회성 이주를 소스 편집 없이 하기 위해서다.
#    `GRADE_LIFT=0.35 python3 patch_cast_cost.py ... --apply` 식으로 쓰고, 반영 뒤에는
#    환경변수를 빼면 자동으로 1.00 으로 돌아온다(소스에 1.30 이 남아 다음 실행이 또 올리는
#    사고를 구조적으로 막는다).
...
def main():
    ...
    print(MEAS.banner(K))
    # DEFAULT_SRC 가 None 이면 srcs 도 None 이라 join 이 터진다(표시 전용 버그).
    src_label = "전체" if (a.all or not srcs) else "/".join(srcs)
    print(f"  캐스트 환율 {cph:.1f} 캐스트/h  ·  대상 {src_label} "
          f"제작가능 {len(pool)}종")
