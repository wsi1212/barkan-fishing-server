...
# ══════════════════════════════════════════════════════════════════════════
#  ★★ DEPRECATED (2026-08-26) — item_ledger.py 가 이 스크립트를 대체했다.
#
#  놓치는 것 넷: ①재료비용이 0이다(가격만 본다) ②**레벨제한을 축으로 쓰지 않아** 사다리
#  붕괴를 잡을 수 없다 ③parts.json 의 유령 원 가격(잠수상점·캐시)을 분리하지 않는다
#  ④작살을 «270 포획/h · income ×1.43 우위»로 가정하는데 실측은 174.8/h · ×1.001 이다
#  (그 가정 위에 세운 「작살 저평가」 결론도 함께 폐기).
#
...
print("\033[33m★DEPRECATED — item_ledger.py 를 쓸 것. 이 출력은 과거 델타 비교 전용이다.\033[0m",
...
OTHER = {"등급특화",   # 내구보존은 2026-08-27 스탯 폐지
          "수중호흡", "수영속도", "공격력", "공격속도", "돌진쿨감", "은신",
         "야간투시"}
