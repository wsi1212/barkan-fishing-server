...
#  ★2026-08-27 유저 정정으로 **False**. "물 밖에 있다가 보이면 가서 빨리잡고 튀는게 정배
#    방식이라 에초에 호흡시간은 수중호흡이 진짜 한 20넘어가지 않는 이상 큰 의미없는거긴해."
#    즉 수색은 물 밖(부두·배·해변)에서 한다. 물속에 있는 건 «접근 + 교전» 뿐이라 잠수 한 번은
#    2.6초 남짓이고, 바닐라 산소 15초만으로 5~6마리가 커버된다 → 호흡시간은 한계효용이 낮다.
SEARCH_IS_SUBMERGED = False
...
#  ★유저: "솔직히 호흡시간때매 익사하는 경우는 거의없음 유저가. ㅂㅅ도 아니고 물밖에
#    나오면 바로 사는건데. 오히려 잠수에서 돈 많이 깎이지." → 익사 비용은 0 으로 두고,
#    돈이 실제로 깎이는 **강제TP** 를 income 에 넣는다(아래 TP_IN_INCOME).
DROWN_DEATH_RATE = 0.0
#: ★강제TP 의 **돈**만 income 에서 뺀다(시간은 실측 사이클에 이미 들어 있어 두 번 세면 안 된다).
#  깊은물 타이머는 «발밑이 물»이면 돈다 — 접근·교전·수면대기가 전부 여기 들어가고,
#  수색만 물 밖이라 빠진다. 세션이 차면 육지로 끌려가고 waterteleport.json 의 cost 를 낸다.
TP_IN_INCOME = True
...
class Model:
    ...
    def income(self, st, dist, wage=None):
        """원/h — 총수입에서 **강제TP 의 돈 손실**을 뺀 값.

        ★2026-08-27 유저 정정으로 두 번 뒤집혔다. 최종 형태의 근거를 남긴다:
          · 익사(호흡시간)는 **안 센다**. "호흡시간때매 익사하는 경우는 거의없음 유저가.
            물밖에 나오면 바로 사는건데." — 맞다. 익사는 플레이어가 회피 가능한 사건이다.
          · 강제TP(수중호흡)는 **돈으로 센다**. "오히려 잠수에서 돈 많이 깎이지."
            깊은물 타이머는 «발밑이 물»이면 도므로 접근·교전·수면대기가 전부 누적되고,
            세션(15 + 수중호흡)이 차면 육지로 끌려가며 waterteleport.json 의 cost 를 낸다.
          · **시간은 빼지 않는다** — 실측 사이클 17.302초에 이미 TP 왕복이 들어 있다.
            시간까지 빼면 두 번 세는 것이고, 실제로 그렇게 했다가 D 급 작살 수입이
            −148,494원/h 가 나왔다.
          · 수색은 **물 밖**이다(SEARCH_IS_SUBMERGED=False) — 정배 플레이가 «밖에서 보다가
            들어가 잡고 나온다»이기 때문. 그래서 물속 체류 = 접근 + 교전 + 수면대기.
        """
        cyc, approach, engage, surface = self.cycle(st, dist)
        per_catch = 0.0
        for g, p in dist.items():
        ...
        tps_h = (catches_h * (approach + engage + surface)) / max(1e-6, self.session_budget(st))
        return gross - tps_h * self.water_cfg()["cost"]
