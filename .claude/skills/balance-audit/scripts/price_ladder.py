...
# ★NEED_TABLE 은 **라이브 자바에서 읽는다**(2026-08-28). 여기 복제해 뒀더니 2026-08-17
#   「Lv70 곡선 재설계」를 반영하지 못하고 옛 값으로 남아 있었다 — Lv20 부터 81개 레벨이
#   어긋났고 Lv70 누적이 225,040 vs 실제 681,717 로 **3.03배** 틀렸다.
#   이 표는 «그 티어에 머무는 시간» 을 정하고, 그게 «구간 수입 총액» → «풀세팅 가격» 으로
#   이어진다. 즉 체류시간이 3배 짧게 잡히면 고티어 가격이 그만큼 싸게 산출된다.
#   2026-08-05 리프라이싱은 그때는 맞았지만 곡선이 바뀌며 근거만 옛 것으로 남았다.
def _live_need():
    import os, re as _re
    root = os.environ.get("BLOCKSHIP_JAVA", os.path.expanduser(
        "~/development/blockship-plugin/src/main/java/com/blockship"))
    src = open(os.path.join(root, "fishing", "FishingLevelManager.java"), encoding="utf-8").read()
    m = _re.search(r"NEED_TABLE = new int\[\] \{(.*?)\};", src, _re.S)
    if not m:
        raise SystemExit("★FishingLevelManager.NEED_TABLE 을 못 읽었다 — 가격 산출 중단"
                         "(복제본으로 조용히 틀리게 도느니 멈춘다)")
    return [int(x) for x in _re.findall(r"\d+", m.group(1))]
...
# ── 소모처 재산정 (2026-08-05) ─────────────────────────────────────────────
# 장비만 올리면 나머지 소모처가 상대적으로 무의미해진다. 다만 **일괄 배수는 틀린 접근**이다 —
# 소모처마다 성격이 다르기 때문:
#   · 마찰비용(사망·워프·대여·홍보) = "잠깐 아픈" 소액. 원래도 크게 어긋나지 않았다(×2~5면 충분).
#   · 자본지출(아이스박스·요리큐) = 장비와 같은 잣대. 여기가 크게 뒤처져 있었다.
#   · 섬/길드 업그레이드 = 이미 종결 수십 시간 규모라 손댈 필요 없다(검산으로 확인).
# 그래서 항목마다 "쓰는 시점의 몇 분/시간 노동인가"를 목표로 잡고 역산한다.
SINKS = [
    # (이름, 현재값, 기준구간, 목표노동(분), 신규값)
    ("사망 페널티 (InnManager.DEATH_COST)",        5000,   "S", 2,   12000),
    ("물 텔레포트 (WaterTeleportManager)",          1000,   "S", 0.5, 3000),
    ("판매글/확성기 (SalePostManager.COST)",        5000,   "S", 2,   12000),
    ("길드 승급 (GuildCommand.PROMOTE_COST)",       5000,   "B", 5,   12000),
    ("스킬트리 리셋 1P (RESET_COST_PER_POINT)",     5000,   "S", 2,   12000),
    ("말 대여 조랑말 (180s)",                        300,    "D", 1,   1500),
    ("말 대여 갈색 여행마 (300s)",                    900,    "D", 3,   4500),
    ("말 대여 백은 준마 (480s)",                     2500,   "B", 6,   13000),
    ("말 대여 흑요석 천리마 (720s)",                  6000,   "B", 15,  33000),
]
# 자본지출 — 최고 티어가 "종결 N시간"이 되도록 곡선 전체를 같은 배수로 민다.
CAPITAL = [
    # (이름, 현재 배열, 배수, 목표 설명)
    ("아이스박스 (IceboxGui.PRICE)",
     [0, 500, 2000, 5000, 15000, 40000, 80000, 150000, 300000, 500000], 3.7, "최고티어 = 종결 5h"),
    ("요리 큐 슬롯 (CookingQueueManager.SLOT_COSTS)",
     [0, 50000, 100000, 200000, 400000, 800000], 1.85, "최고티어 = 종결 4h"),
]
# 손대지 않는 것 — 검산 결과 이미 스케일이 맞는다.
CAPITAL_KEEP = [
    ("섬 업그레이드 7종 합계", 22_950_000),
    ("길드 업그레이드 5종 합계", 25_500_000),
    ("+15 강화 명목비용", 8_557_000),
]
# 퀘스트 보상금 — 필요레벨 구간별 배수. 초반은 이미 물가와 얼추 맞고(중위 8,000 = D티어 5분),
# 고레벨 퀘가 크게 뒤처져 있었다(50,000 = 종결 8분인데 A부품 1개가 660,000).
QUEST_MULT = [(10, 1.5), (20, 2.5), (40, 4.0), (999, 8.0)]


def sink_report(rows):
    inc = {r["grade"]: r["won_h"] for r in rows}
    print("\n" + "=" * 78)
    print("소모처 재산정 — 마찰비용 (쓰는 시점의 노동시간 기준)")
    print("=" * 78)
    print(f"{'항목':<40}{'현재':>10}{'신규':>12}{'배':>6}{'신 노동':>10}")
    print("─" * 78)
    for name, cur, stage, target_min, new in SINKS:
        won_min = inc[stage] / 60.0
        print(f"{name:<40}{cur:>10,}{new:>12,}{new/cur:>5.1f}x{new/won_min:>9.1f}분")

    print("\n" + "=" * 78)
    print("소모처 재산정 — 자본지출")
    print("=" * 78)
    for name, arr, mult, why in CAPITAL:
        new = [0] + [int(round(v * mult / 1000.0) * 1000) for v in arr[1:]]
        top_h = new[-1] / inc["S"]
        print(f"\n{name}  (×{mult}, {why})")
        print(f"  구: {', '.join(f'{v:,}' for v in arr)}")
        print(f"  신: {', '.join(f'{v:,}' for v in new)}")
        print(f"  최고티어 노동: {arr[-1]/inc['S']:.2f}h → {top_h:.2f}h")

    print("\n유지(이미 스케일 정상 — 검산):")
    for name, total in CAPITAL_KEEP:
        print(f"  {name:<28}{total:>14,}원 = 종결 {total/inc['S']:>6.1f}h")
