...
# ★레일 배율 — oversized_in_gui 로 클리핑이 풀린 뒤 1.7은 **너무 길어서 노드 링을
#   파고들었다**(2026-08-06 실사고). 기하: 레일은 타일이 칸을 꽉 채우므로(f=1.0)
#   슬롯 밖으로 나가는 길이 = 8·s − 9 px. 노드 링 바깥끝은 자기 슬롯 경계보다
#   0.34px 안쪽(8·1.332·0.8125 = 8.66 < 9)에 있다.
#   s=1.7 → 4.6px 삐져나가 링을 4.26px 침범. s=1.3 → 1.4px 삐져나가 링과 1.06px만
