...
def farm_scarecrow(seed=0):
    ...
    k.box((5.0, 12.0, 5.45), (8.2, 15.2, 5.9), patch, rot=("z", -22.5))
    k.box((3.8, 10.0, 5.35), (12.2, 11.2, 10.3), rope)

    # 목수건은 목을 한 바퀴 감싼 띠 + 정면 매듭 + 긴 꼬리로 분리해 덧댐과 혼동되지 않게 한다.
    k.box((5.5, 19.25, 5.1), (10.5, 20.65, 10.9), scarf)                     # 목을 감싼 띠
    k.box((7.0, 18.0, 4.72), (9.0, 19.75, 5.65), scarf)                      # 앞 매듭
    k.box((6.25, 14.1, 4.75), (7.85, 18.4, 5.65), scarf, rot=("z", 22.5))   # 길게 늘어진 꼬리
    # 단추는 얼굴색과 겹치지 않는 밝은 나무색으로 가슴 중앙에 세로 배치한다.
    k.box((8.55, 15.2, 5.18), (9.4, 16.05, 5.72), rope)                      # 단추 1
    k.box((8.55, 12.55, 5.18), (9.4, 13.4, 5.72), rope)                      # 단추 2
    ...
    # 자루를 꿰맨 세로 봉제선과 세 바늘땀. 표정을 건드리지 않고 빈 오른뺨만 채운다.
    k.box((10.45, 21.0, 4.05), (10.85, 26.35, 4.5), dark)
    k.box((10.05, 22.0, 4.0), (11.2, 22.35, 4.52), rope)
    k.box((10.05, 23.25, 4.0), (11.2, 23.6, 4.52), rope)
    k.box((10.05, 25.3, 4.0), (11.2, 25.65, 4.52), rope)
