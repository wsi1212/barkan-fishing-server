...
# ═══════ 농사 설비 — 바닐라 작물 오프라인 성장 허수아비 ═══════

def farm_scarecrow(seed=0):
    """밀짚 모자·자루 얼굴·기운 누더기 옷을 가진 2블록 높이 설치형 허수아비."""
    k = Kit(seed)
    wood = Mat("765033", var=0.8, grain="v")
    straw = Mat("cb9753", var=0.9, grain="v")
    sack = Mat("b8874f", var=0.8, grain="v")
    hat = Mat("d3a34f", var=0.8, grain="v")
    cloth = Mat("4f6670", var=0.9, grain="v")
    patch = Mat("8d4d35", var=0.8)
    scarf = Mat("a84f3f", var=0.75, grain="v")
    rope = Mat("9d7642", var=0.7, grain="v")
    dark = Mat("2b2520", var=0.35)

    # 땅에 박힌 T자 골격 — 옷 틈에서 실제 나무가 보여 평면 인형처럼 보이지 않는다.
    k.box((7, 0, 7), (9, 23, 9), wood)
    k.box((-3, 16, 7), (19, 18, 9), wood)

    # 누더기 셔츠: 두꺼운 몸통·소매·비대칭 자락으로 진짜 부피를 만든다.
    k.rounded_box((4, 10, 5.8), (12, 20, 10.2), cloth, bevel=1)
    k.box((-0.5, 14.7, 5.8), (5, 19.1, 10.2), cloth, rot=("z", -22.5))
    k.box((11, 14.7, 5.8), (16.5, 19.1, 10.2), cloth, rot=("z", 22.5))
    k.box((3.8, 7, 6.0), (7.5, 11.2, 10.0), cloth, rot=("z", 22.5))
    k.box((7.3, 7.7, 6.0), (12.3, 11.2, 10.0), cloth, rot=("z", -22.5))
    k.box((4.3, 11.8, 5.45), (6.9, 14.6, 5.9), patch, rot=("z", -22.5))
    k.box((3.8, 10.0, 5.35), (12.2, 11.2, 10.3), rope)

    # 목수건은 각 조각을 넉넉히 겹친 연속 구조다: 목띠→양쪽 접힘→매듭→연결부→긴 꼬리.
    k.box((5.3, 19.3, 5.0), (10.7, 20.7, 11.0), scarf)                       # 목을 감싼 띠
    k.box((5.4, 18.65, 4.35), (8.1, 20.15, 5.35), scarf,
          rot=("z", -22.5, (8.0, 19.5, 4.85)))                              # 왼쪽 접힘
    k.box((7.9, 18.65, 4.35), (10.6, 20.15, 5.35), scarf,
          rot=("z", 22.5, (8.0, 19.5, 4.85)))                               # 오른쪽 접힘
    k.box((6.7, 17.65, 4.15), (8.3, 19.45, 5.3), scarf)                      # 앞 매듭
    k.box((5.9, 16.25, 4.2), (7.15, 18.35, 5.3), scarf,
          rot=("z", 22.5, (7.0, 18.0, 4.75)))                               # 매듭-꼬리 연결부
    k.box((4.85, 13.65, 4.25), (6.5, 16.95, 5.3), scarf,
          rot=("z", 22.5, (6.2, 16.7, 4.75)))                               # 길게 늘어진 꼬리
    # 단추는 얼굴색과 겹치지 않는 밝은 나무색으로 가슴 중앙에 세로 배치한다.
    k.box((7.55, 15.2, 4.55), (8.45, 16.1, 5.12), rope)                      # 단추 1 — x=8 중심
    k.box((7.55, 12.5, 4.55), (8.45, 13.4, 5.12), rope)                      # 단추 2 — x=8 중심
    k.box((1.0, 16.0, 5.35), (3.6, 18.3, 5.85), patch, rot=("z", -22.5))   # 팔꿈치 덧댐

    # 소매 끝의 밀짚 손 — 작은 손가락 가닥은 아이콘에서 잡음처럼 보여 단순한 뭉치로 정리한다.
    k.box((-3.4, 15.1, 6.2), (0.2, 18.6, 9.8), straw)
    k.box((15.8, 15.1, 6.2), (19.4, 18.6, 9.8), straw)
    k.box((-0.7, 14.9, 5.8), (0.55, 18.9, 10.2), rope)                       # 왼 손목 묶음
    k.box((15.45, 14.9, 5.8), (16.7, 18.9, 10.2), rope)                     # 오른 손목 묶음

    # 자루 얼굴과 목. 얼굴 장식은 북쪽(z-)으로 살짝 돌출시켜 어느 거리에서도 읽힌다.
    k.box((6.5, 19.0, 6.5), (9.5, 22.0, 9.5), straw)
    k.rounded_box((4.5, 20.5, 4.5), (11.5, 27.5, 11.5), sack, bevel=1)
    # 눈은 큰 검은 사각형이 아니라 자루에 꿰맨 가는 실눈이다(단추눈처럼 보이지 않게).
    k.box((5.25, 24.65, 4.08), (7.25, 25.2, 4.58), dark, rot=("z", -22.5))
    k.box((8.75, 24.65, 4.08), (10.75, 25.2, 4.58), dark, rot=("z", 22.5))
    k.box((7.45, 22.7, 3.75), (8.65, 24.2, 4.65), patch, rot=("x", -22.5))
    k.box((6.0, 21.5, 4.05), (8.0, 22.15, 4.6), dark, rot=("z", -22.5))
    k.box((7.8, 21.25, 4.05), (10.3, 21.9, 4.6), dark, rot=("z", 22.5))

    # 밀짚모자: 챙과 크라운을 따로 회전시키지 않는다. 이전 형태는 각자의 중심축으로 돌아가
    # 층이 어긋나고 정면에서 챙이 얼굴을 대각선으로 가르는 문제가 있었다.
    k.box((2.0, 26.5, 4.0), (14.0, 26.85, 12.0), rope)                       # 챙 밑그늘
    k.box((1.5, 26.8, 3.5), (14.5, 27.7, 12.5), hat)                         # 얇고 넓은 주 챙
    k.box((2.5, 27.6, 4.2), (13.2, 28.2, 11.8), hat)                         # 챙 윗단
    k.box((4.5, 27.7, 5.0), (11.5, 31.2, 11.0), hat)                         # 곧은 크라운
    k.box((5.3, 31.2, 5.7), (10.8, 32.0, 10.3), hat)                         # 살짝 오므린 윗면
    # 띠는 큰 상자로 덮지 않고 네 면만 감싸 모자 부피와 밀짚색이 그대로 보이게 한다.
    k.box((4.35, 28.0, 4.72), (11.65, 29.0, 5.18), patch)                    # 앞
    k.box((4.35, 28.0, 10.82), (11.65, 29.0, 11.28), patch)                  # 뒤
    k.box((4.28, 28.0, 5.0), (4.72, 29.0, 11.0), patch)                      # 왼쪽
    k.box((11.28, 28.0, 5.0), (11.72, 29.0, 11.0), patch)                    # 오른쪽
    # 햇볕에 닳아 덧댄 모자 조각 + 굵은 두 땀.
    k.box((8.25, 29.35, 4.62), (10.7, 30.8, 5.08), cloth, rot=("z", -22.5))
    k.box((8.35, 29.6, 4.5), (9.0, 29.95, 5.12), rope, rot=("z", -22.5))
    k.box((9.8, 30.25, 4.5), (10.45, 30.6, 5.12), rope, rot=("z", -22.5))
    return _build(k)

REGISTRY["farm_scarecrow"] = (farm_scarecrow, {})
