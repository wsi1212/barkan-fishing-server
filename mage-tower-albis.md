...
> - ★**오로라 날씨 자체는 그대로 둔다** (2026-08-14 확정). `WeatherManager`의 날씨 타입 「오로라」
>   (전역·심야 커튼, 경험치+15%·크리확률+20%), `AuroraDisplayManager` 리소스팩 커튼,
>   `RegionParticleTask` 파티클은 **마탑과 무관한 독립 밤 이벤트**다 — 건드리지 말 것.
>   **지우는 것은 퀘스트 쪽뿐**이다: 의식 라인 + `FishingListener` 강제 소환 훅.
...
### 1. 좌표 추적 퀘스트라인 (7챕터 4막 · 메인)
- 레벨 게이트: **Lv62** — `심해19`(이름을 읽다) 직후.
- 서사: 대도서관이 이름을 지우면서 **자리까지 지웠다.** 기록이 없으니 **물에게 묻는다.**
- 흐름: `알비스00`(도개교 열쇠) → `알비스01`(신선한 표본 6) → `알비스02`(성광어 1) → `심해20`(협곡 도착).
- 보상: 도감 등록(성광어) + **포탈망 각성 플래그** + 협곡 좌표(서사).
- ★**오로라는 쓰지 않는다.** 옛 「오로라 의식」 안은 폐기됐다.

### 1-B. 도개교 — 마탑은 잠겨 있다 ★2026-08-14
- 마탑 앞 성문의 **도개교가 올라가 있다.** 알비스가 제 손으로 걸어 잠갔다.
- 열쇠는 하나뿐이고 **세르간**이 갖고 있다 — 둘은 옛 동료다(2챕터 인물 회수).
- `알비스00` 완료 시 `보상열쇠 "마탑도개교:마탑 도개교 열쇠"`.
- ★**규약(door/LockedDoorManager)**: 열쇠는 실물 아이템이 아니라 `extraFlags["열쇠"]`의 문자열이고,
  **문은 물리적으로 열지 않는다** — 클릭 이벤트 취소 + 열쇠 보유자만 안쪽으로 개별 TP.
  도개교는 올라간 채로 두고 통과만 시킨다(다른 유저가 따라 들어오는 것을 막는 게 설계 의도).
- OP 설정: `/잠긴문 pos1/pos2 → 생성 마탑도개교 마탑도개교 마탑 도개교 열쇠 → 도착점 마탑도개교`.
...
## 전설 어종 — 성광어(星光魚)
- **균열에서 새어 나온 빛**을 먹고 사는 심해어. 하늘빛이 아니다.
- **협곡 주변 해역에서만** 낚인다 → 그 물고기가 잡히는 자리가 곧 균열 위다. **물고기가 나침반**이다.
- ★옛 「오로라 활성 중 마탑 해역」 조건은 폐기. 어종은 그대로 재사용하고 서식 조건만 바꾼다.

## 기본값 요약 (튜닝 가능)
| 항목 | 값 |
|------|-----|
| 퀘 레벨 게이트 | **Lv62** (7챕터 4막) |
| 포탈 사용 | 무료 + 30초 쿨다운 |
| 포탈 거점 | 마탑/스폰, 사막마을, 상단마을, 왕도 |
| 전설 어종 | 성광어(星光魚), **협곡 해역 전용** |
| 진입 | **도개교** — 「마탑 도개교 열쇠」(`알비스00` 보상) |
...
- 성광어: grade E(신뢰 포획), 게이트 = `quest:"알비스02"` + **협곡 해역** 서식.
- ~~의식 오로라 전용 시작 메서드~~ → **불필요.** 폐기(2026-08-14). 구현이 그만큼 가벼워진다.
- 마탑 신규 지역 생성(좌표 실측 필요).
- 퀘라인: 알비스00(열쇠) → 알비스01(신선 표본 6) → 알비스02(성광어 1 → 클레임 → `보상각인` 포탈각성 + `보상도감해금` 성광어).
- 포탈 패드: Portal/Ferry 확장 아닌 **신규 PadManager**(박스진입 트리거 + 페리식 선택 GUI + tp 디스패치 + `pads.json` + 각인 게이트).

**트랙별 작업:**
- 【JSON】 fish.json(성광어 서식=협곡 해역), regions.json(마탑), quests.json(알비스00/01/02 — **생성기가 쓴다**), npc.json+dialogue.json(알비스), pads.json(4거점), locked-doors.json(도개교).
- 【Java】 ~~QuestManager 보상각인·PadManager~~ **구현 완료**(`QuestManager:1541` · `PadManager.ATTUNE_PREFIX`).
  남은 것 = **`FishingListener:158~160` 오로라 소환 훅 제거** + 성광어 서식 게이트 교체.
...
- ~~★결정적 제약: `visit`/`action`은 플래그 스킵~~ → **2026-08-14 소스 재확인 결과 낡은 정보다.**
  `completeTravel`이 `rewardQuestContents`를 호출한다(`QuestManager:1141`) — 방문 퀘스트도 칭호·열쇠·각인을
  전부 지급한다. 코드 주석에 *"예전에는 돈/경험치만 처리해 튜토_이동6의 별빛진주가 누락됐다"* 고 수정 이력이 적혀 있다.
  → 그래서 `알비스00`(visit) + `보상열쇠` 조합이 성립한다.
...
- **퀘라인(3단계, 확정)**: 알비스00(열쇠) → 알비스01(신선 표본) → 알비스02(성광어 → 각성).

### fishing/ + fish.json (조사 완료)
- **fish.json**: `{fish:{name→FishDef}, regions:{rid→{sublist→[names]}}, environment:{type→[names]}}`. FishDef = `minSize maxSize grade(E~S) time weather quest`. 로더 `region/FishLoader`(단일 권위). 도감 소속 = regions/environment 어딘가에 이름 있으면 분류. 선례 퀘물고기 `상단화물꼬리표`=grade E(=신뢰 포획)+`quest` 게이트 → **성광어도 낮은 grade로 두면 협곡 해역에서 확실히 잡힘**(전설=로어, 포획난이도 아님).
...
> ### ⛔ 아래 조사·구현 기록은 폐기된 오로라 안의 것이다 (2026-08-14)
> 지우지 않고 남기는 이유는 **코드에 실제로 들어간 것들이라 제거 대상을 추적해야 하기 때문**이다.
> 새 설계는 위 §1 「좌표 추적 퀘스트라인」이 권위다.
