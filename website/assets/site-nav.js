(() => {
  ...
  host.innerHTML = `<nav class="barkan-site-nav" aria-label="주 메뉴"><a class="barkan-site-brand" href="/"><strong>BARKAN</strong><small>ISLANDS</small></a><div class="barkan-site-links" role="menubar">${desktopGroups}</div><a class="barkan-site-direct" href="/law">법전</a><div class="barkan-site-tools"><a class="barkan-site-profile" data-auth-link href="/community/login">로그인</a><a class="barkan-site-profile barkan-site-logout" data-logout-link href="/community/logout" hidden>로그아웃</a><button class="barkan-site-menu-toggle" type="button" aria-expanded="false" aria-controls="barkan-site-mobile-menu"><i aria-hidden="true"><span></span><span></span><span></span></i><b>메뉴</b></button></div><div class="barkan-site-mobile-menu" id="barkan-site-mobile-menu" hidden>${mobilePrimary}${mobileGroups}</div></nav>`;
  ...
  fetch('/community/session', {credentials: 'same-origin', cache: 'no-store'})
    ...
    .then(session => {
      ...
      const logoutLink = host.querySelector('[data-logout-link]');
      if (!authLink || !logoutLink || !session?.authenticated) return;
