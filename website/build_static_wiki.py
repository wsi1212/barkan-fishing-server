...
DOCS = [
    {
        ...
        "sections": [
            ("접속 정보", ["서버 주소는 barkan.kr입니다. Java Edition과 Bedrock Edition(Geyser 크로스플레이)을 지원합니다.", "접속 가능 버전은 1.21.11부터 1.26.2까지입니다."], []),
    ...
    },
    {
        ...
        "sections": [
            ...
            ("천장(PRD) 계산법", ["등급 X의 현재 판정 확률은 기본 확률 × (1 + 해당 등급의 미출현 횟수)입니다. 첫 시도는 기본 확률이고, 한 번 빗나갈 때마다 기본 확률만큼 확률이 더해집니다. 해당 등급이 나오면 그 등급의 카운터는 0으로 돌아가고, 다른 등급 카운터는 1씩 올라갑니다."], []),
        ...
        "tables": [
            {
                "title": "등급별 실질 출현율과 천장 상승폭",
                "headers": ["등급", "기본 확률", "실질 출현율*", "평균 시도", "미출현 1회당 상승폭"],
                "rows": [
                    ["G", "0.0000175%", "약 0.033%", "약 3,000회", "+0.0000175%p"],
                    ["L", "0.0021%", "약 0.36%", "약 275회", "+0.0021%p"],
                    ["M", "0.0105%", "약 0.82%", "약 122회", "+0.0105%p"],
                    ["S", "0.0165%", "약 1.02%", "약 98회", "+0.0165%p"],
                    ["A", "0.0977%", "약 2.48%", "약 40회", "+0.0977%p"],
                    ["B", "0.5712%", "약 6.01%", "약 17회", "+0.5712%p"],
                    ["C", "7.13%", "약 21.47%", "약 4.7회", "+7.13%p"],
                    ["D", "21.12%", "약 33.22%", "약 3.0회", "+21.12%p"],
                    ["E", "잔여 확률", "약 34.57%", "약 2.9회", "해당 없음"],
                ],
                "note": "* 실질 출현율은 GradeRoller 현재 롤 순서와 PRD를 500만 캐스트로 시뮬레이션한 값입니다(Lv.100·모든 등급 풀·행운 0·등급 특화 없음). 지역에 해당 등급 어종이 없거나 레벨 게이트가 닫혀 있으면 그 등급 롤은 건너뛰며, 피티는 보존됩니다. %p는 퍼센트포인트입니다.",
            },
        ],
    },
    {
...
def page_html(doc: dict[str, object], docs: list[dict[str, object]]) -> str:
    ...
    if tables:
        tables = "\n      " + tables
    content = f"{tables.lstrip()}{sections}" if doc["slug"] == "fishing" else f"{sections}{tables}"
    faq = ""
    if doc.get("faq"):
